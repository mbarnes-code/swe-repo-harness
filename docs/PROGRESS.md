...
1. **Nothing in the harness could clear a sticky budget halt** (D54). `git grep` found exactly one
   ...
   0` — so §10's documented exit-3 recovery was a **permanent no-op**. Fixed and landed (`c45db53`).
...
8. **`record_attempt`'s INSERT omits five columns** (D62) — `state/repository.py:1677-1690` names 24
   ...
   `llm_cache_hit`. Re-verified on `main`; **open**.
...
3. **`attempts.llm_failovers` and four sibling columns still have no writer** (defect 8 above;
   ...
   without changing `WorkerError` or `TierUnavailable`'s raise sites. (D62.)
...
**The amendment window, `b1de36e..87ed419`.** Measured against `main` at **`87ed419`**: `git rev-list --count 6a5e534..87ed419` → **61**, cross-checked `git log --oneline 6a5e534..87ed419 | wc -l` → **61**, of which **24** landed after `b1de36e` (`git log --oneline b1de36e..87ed419 | wc -l`). **34 of the 61 touch `src/` or `tests/`** (`git rev-list --count 6a5e534..87ed419 -- src/ tests/`), so **27 are documentation only**; the code delta from the round base is **+5,022 / −42 across 20 files** (`git diff --stat 6a5e534..87ed419 -- src/ tests/`), up from the `+2,998 / −25` this section first recorded. **The tree moved under this amendment three times, and the drafts it invalidated are why this paragraph exists**: `HEAD` was `d2e0090` when it began, `cfd89c7` when the first figures were taken, and `87ed419` when they were re-taken — four commits (`8df4af8`, `c6bdd26`, `9955583`, `87ed419`) landed in between, and **three of the four** falsified something a draft of this amendment had recorded as open (the fourth repoints a citation). Every one was caught by re-reading `git log`, exactly as this section's original measurement-window disclosure says to. `git status --short` at `87ed419` shows **only the untracked `.superpowers/` scratch directory and this file** — no lane's edits are uncommitted. **`ruff check src/ tests/` under `.venv/bin/python` passes**, re-run by me at `87ed419` itself after `8df4af8` changed `src/` under an earlier run of it (`cf85c8b` had fixed the two outstanding errors). **Still expect commits past `87ed419`**, and check `git log` before trusting any "open" below.
...
**Landed in the amendment window** (`b1de36e..87ed419`; verified against `git log`, not against the controller's ledger, which stops at the session limit).
...
24. **HONEST2 — D73, LANDED** (`cfd89c7`). `ContainerSandbox.list_with_verdict` returns a `ContainerListing` that keeps *"this run has no containers"* apart from *"docker did not tell me"*, `reap()` reads it and reports a failed listing as a `failed` entry naming the prefix rather than as a clean sweep, and `list_by_prefix` is **kept, lenient, and documented as the residual** (`container.py:452-468`). Verified at this anchor by reading the file, not the report. The **D73 ledger entry landed separately at `9955583`**, after the code, under a `D72–D73` heading that also records the real-daemon test hazard.
...
- **11 registered workers** — `grep -rn "@register_worker" src/fleet/workers/ | wc -l`. `cli.py:10044` at `HEAD` still says "All twelve workers".
...
5. **Zero D-numbers were allocated for ten pre-existing defects.** Two disclosures in `docs/INTEGRATION_HONESTY.md` explicitly decline a number and say `D71` stays free; ADR-0082's boundary was flagged as possibly ledger-worthy with no number allocated. Round D should decide whether this round's defects enter the ledger or are deliberately left out of it. **AMENDED at `87ed419`: partly answered, and the ledger is now deliberately non-contiguous.** **D72 and D73 were allocated centrally at dispatch and landed** at `9955583`, with `87ed419` pointing D72 at its design doc; the section heads at `docs/INTEGRATION_HONESTY.md:4143` and states its own arithmetic rather than assuming it — **`D71` was never allocated and remains free**, because the disclosures at `:4008`, `:4052` and `:4139` each explicitly decline a number, and the new section refuses to close that gap by quietly taking it. Highest allocated is now **D73**, with **D71 free**: the four earlier next-free-number sentences at `:3267`, `:4008`, `:4052` and `:4139` are still true as written, and **a reader who assumes contiguity will misread the file**. **`D74` is not allocated** — it appears only in the controller's real-time ledger, as a number a lane expected to owe. **The ten pre-existing defects listed above still carry no numbers**, so the original question stands for them, narrower by two.
