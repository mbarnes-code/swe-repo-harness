...
`ecosystems/contracts/`, the `ContractAdapter` registry of ADR-0019 + ADR-0020) is a defect,
enforced by §12.6. [2026-09-06, round VI research-34: corrected — the parenthetical above used to
read "**NOT YET IMPLEMENTED**, ADR-0065: that subpackage does not exist, and the per-`ContractKind`
knowledge it would hold currently lives as three `Mapping[ContractKind, …]` tables in
`workers/contracts.py`". True through round VI task 19, falsified by task 40 (`f3c0200`), which
shipped all five adapters — `ecosystems/contracts/` exists and is the real registry now. §12.6's
own enforcement test (`tests/test_ecosystems.py`) currently has 3 live, unresolved reds against
this exact rule from two later commits (round VI tasks 53/56) that branched on `Ecosystem`/
`ContractKind` outside the adapter packages — see `D120` (`docs/INTEGRATION_HONESTY.md`) for the
open defect, not fixed by this correction.] Contract kinds
...
3. **Coordinate normalization.** `adapter.parse()` yields `RawDependency`; `adapter.coordinate()`
   ...
   **Two repos publishing the same coordinate** is not a tie to be broken silently.
   `coordinates.coord_key` is the primary key and holds ONE `owner_repo_id`, so that table cannot
   express the contest at all; §3.1 step 8's detector reads the claims from `manifests` instead.
   The insert is `ON CONFLICT (coord_key) DO UPDATE SET owner_repo_id = COALESCE(excluded.owner_repo_id, coordinates.owner_repo_id)` — a promotion-only merge — and it writes no `collisions` row of its own: the row of `kind = 'COORDINATE'` naming both `repo_id`s is written by `fleet sequence`'s step 8 audit (`src/fleet/cli.py:1602-1608` is the upsert and the only `INSERT INTO coordinates` in `src/`; `:2952-2957` is the audit). **[Corrected 2026-08-28, round-L lane W8, base `a13a971`; `src/fleet/cli.py` line numbers re-derived against `a8f52c9` by round-L lane W9, `cli.py` having gained 5 lines at `:1759` since the base — each re-derived by matching the cited text, not by adding 5. This passage read: *“`coordinates.coord_key` is the primary key, so the second writer would clobber the first; instead the insert is `ON CONFLICT DO NOTHING` and a `collisions` row of `kind = 'COORDINATE'` is written naming both `repo_id`s”* — the retired words are quoted here so a sweep for them finds this correction and not a survival. `DO NOTHING` is not merely absent from the code, it is the wrong rule: a coordinate first written owner-less as some other repo's dependency would keep `owner_repo_id IS NULL` for the rest of the run, `cli.py::_owner_index` (`:2526-2537`) selects `WHERE owner_repo_id IS NOT NULL`, and the coordinate would read EXTERNAL — so the ordering edge its publisher earns would never be inferred. Third of the three sites that carried this claim; the other two are §6's idempotency table and §13 row 6, all three corrected in one commit. Clause (iii) below is NOT part of this correction and is not certified by it — see §13 row 6's marker. §12.27's marker carries the per-detector measurement.]** Resolution policy, in order:
...
6. **DAG build & SCC detection** (`graph/cycles.py`). `networkx.DiGraph` built from `edges` on
   ...
   **Rollback is one procedure for both, and it is cheap because 5b never mutated anything
   ...
   exclude the failed contract's `CONTRACT_IMPL`/`CONTRACT_CONSUME` edge rows from the next graph
...
2. **BUILD generation** (`workers/buildgen.py`, the adapter driver; text rendering in
   ...
   **Contract nodes: generated code is regenerated, never copied.** This is the consequence that
   ...
   `ContractAdapter`, keyed by `ContractKind` (§7.6) — implemented as of round VI task 40
   (`f3c0200`; ADR-0065 shipped the mechanism, task 40 shipped the last four adapters): the
   pseudocode below now describes this tree, not just its design. [2026-09-06, round VI
   research-33: corrected — the prior "NOT YET IMPLEMENTED, no `ecosystems/contracts/` package
   exists" text was true 2026-09-03, falsified by task 40, and was self-contradicted by this same
   section's own table eleven lines below even before this correction.]
...
class RepoState(FleetModel):
    ...
    blocked_by: list[RepoId] = Field(
        ...
        "which writes SKIPPED (§10), are LIVE — measured at `90d423d`, runner._contain, "
        "cli._quarantine_impl and cli._repropagate_terminal_providers (§3.5), which re-fires the "
        "SAME RHI trigger across a process boundary a later invocation's own pre-seed pass cannot "
        "reach on its own (D126/ADR-0130, round VI task 84), are the only non-delegating callers "
        "in `src/`. Non-delegating means "
...
CREATE TABLE IF NOT EXISTS findings (             -- cycles, no-manifest, preflight, conflicts
    ...
    kind       TEXT NOT NULL,                     -- FREE TEXT, deliberately: findings are raised by
                                                  ...
                                                  -- | 'HoistRollbackFailed' (fix round, task 72
                                                  --   controller review I2: one row per contract
                                                  --   whose rollback attempt raised, repo_id
                                                  --   NULL)
...
| `coordinates` | `coord_key` | `DO UPDATE SET owner_repo_id = COALESCE(excluded.owner_repo_id, coordinates.owner_repo_id)` — a promotion-only merge, never `DO NOTHING`, which would strand a coordinate first seen as another repo's dependency as EXTERNAL for the rest of the run and so delete the ordering edge its publisher earns (§3.1 step 3). **No `collisions` row is written here**; the `COORDINATE` audit is §3.1 step 8's, and it runs in `fleet sequence`. **[Corrected 2026-08-28, round-L lane W8, base `a13a971`; `src/fleet/cli.py` line numbers re-derived against `a8f52c9` by round-L lane W9, `cli.py` having gained 5 lines at `:1759` since the base — each re-derived by matching the cited text, not by adding 5: this cell read *“`DO NOTHING` + a `collisions` row (§3.1 step 3)”*, quoted here so a sweep for it finds this correction and not a survival. Neither half held: `src/fleet/cli.py:1602-1608` is the only `INSERT INTO coordinates` in `src/` and it is a `DO UPDATE`, and the only two `audit_collisions` callers in `src/` are `cli.py:2952-2957` (`fleet sequence`) and `src/fleet/workers/contracts.py:372-379`. Second of the three sites that carried this claim; see §12.27's marker.]** |
...
**[Marker 2026-08-28, round-K lane W4 — `D86`: this mechanism does not exist.** `initializer=` occurs zero times in `src/` and `tests/`; the tree's only `ProcessPoolExecutor(` (`src/fleet/orchestrator/budgets.py:947-949`) passes `max_workers=` and `mp_context=` only. §12.47's copy of this sentence is **retired**, not deferred, and the reason is in that criterion's marker: the pool's only submitted callable (`scan_file`, via `src/fleet/workers/symbolindex.py:266-268`) touches no registry, and an initializer would turn a startup `RuntimeError` into a `BrokenProcessPool`. The sentence below is left standing rather than rewritten, because retiring it in design prose is an adjudication `D86` records and this repair does not make. Do **not** add `initializer=` to satisfy it.]**
...
"this language does not consume this IDL", which is a finding, not a crash (§13 row 31). [2026-09-06,
round VI research-34: corrected — the prior text here read "*That consumer does not exist yet — see
§7.6's NOT YET IMPLEMENTED note.*", true through round VI task 19, falsified by task 40 (`f3c0200`)
and task 56 (`57d7171`, `docs/SPEC.md`'s own §12.34 Clause B work), which wired a real reader:
`src/fleet/cli.py`'s `fleet build` PASS 2b reads `contract_bindings.get(cnode.kind)` directly.]
...
registry. **[Marker 2026-08-28, round-K lane W4 — `D86`: the `cpu_pool` half of that sentence is false; no pool initializer exists. See §7.2's marker and §12.47's.]** `for_ecosystem(eco)` is total by construction after `discover()` and raises only if
...
> [2026-09-06, round VI research-34: corrected — the block below was true through round VI task 19
> and is FALSE at current `HEAD`, falsified by task 40 (`f3c0200`), which shipped the ABC and all
> five adapters, and by task 56 (`57d7171`), which wired `contract_bindings` a real reader. Kept
> per the annotate-never-rewrite discipline rather than deleted; everything the block below
> describes is now description, not only design.]
>
> **NOT YET IMPLEMENTED (ADR-0065), historical — see the correction above.**
> `src/fleet/ecosystems/contracts/` does not exist in this
...
src/fleet/
  ...
  ecosystems/                 # the OUTPUT-side language boundary (ADR-0020); §7.5
    ...
    contracts/                # §7.6, keyed by ContractKind, not by Ecosystem. Shipped in full
                              #   (round VI tasks 19+40, f3c0200) -- [2026-09-06, round VI
                              #   research-34: corrected -- this comment block used to describe
                              #   the subpackage as NOT YET IMPLEMENTED / not existing; that was
                              #   true through round VI task 19, false since task 40.]
  ...
  llm/
                              #   *** MARKER 2026-08-28 (round-K repair of `12be741`, lane W4) ***
                              #   FOUR modules listed under llm/ DO NOT EXIST: routing.py,
                              #   negotiate.py, failover.py, capabilities.py. ONE module that
                              #   exists is not listed: roles.py, which holds the RoleRouter.
                              #   (__init__.py is omitted throughout this listing by convention,
                              #   so it is not counted as an omission.) The reader of
                              #   config/models.yaml is settings.py, not routing.py -- see §12.40.
                              #   docs/PROGRESS.md records the absence TWICE and the two records
                              #   DISAGREE: :5542 "none exists, none should be created" names
                              #   routing/negotiate/capabilities; :780 names failover.py; :777
                              #   calls it "a rename plus import update". Whether these modules
                              #   get built or this listing gets retired is an ADJUDICATION, left
                              #   open on purpose. Do not repoint the listing silently.
...
budgets:                      # §11.2, all durable in budget_ledger / repo_ledger
  ...
  run_max_llm_calls:          # unset => no cap. The call-count ceiling (ADR-0142), orthogonal to
                              #   run_max_cost_usd and enforced by the same durable CAS. Set it on
                              #   a `price: free` profile, where every call costs $0.00 and the
                              #   dollar ceiling above is therefore structurally inert. Breach →
                              #   exit code 3, sticky through the CAS predicate (NOT via halted=1)
...
scan:
  ...
  api_contract_patterns:      # API_CONTRACT edge inference (advisory, gRPC/proto + HTTP reference side)
    grpc_method_path: "[\"']/((?:[A-Za-z_]\\w*\\.)+[A-Za-z_]\\w*)/[A-Za-z_]\\w*[\"']"
    http_path_template: "(?<![fFrRbB])[\"'`](/(?:[\\w.-]+/)*\\{[A-Za-z_]\\w*\\}(?:/(?:[\\w.-]+|\\{[A-Za-z_]\\w*\\}))*)[\"'`]"
...
| `fleet quarantine` | Remove one pathological repo from the fleet mid-run **without editing config**: writes an audited `findings` row of kind `OperatorQuarantine` with the required reason, sets `RepoStatus.SKIPPED`, and propagates `blocked_by` to its dependents exactly as an abandonment does. Exists because the alternative — editing `config/repos.yaml` — trips config-drift detection whose only escape silently accepts every other co-edited change too. **Marker 2026-09-15 (round VIII §15.1 item 3, D141) — `--stub-blocked` below is accepted but not yet wired.** `quarantine()` parses the flag and discards it (`_ = stub_blocked`, `src/fleet/cli.py:16397`); `_quarantine_impl` has no such parameter, so an operator passing `--stub-blocked` gets no error and no effect. This row's documented flag list is the target state and is unchanged by this marker (Rule 14); see `docs/INTEGRATION_HONESTY.md` D141 for the gap and the two remediation options it names. | `<repo>` `--reason TEXT` (required) `--stub-blocked` `--dry-run` |
| `fleet retry` | Reopen one abandoned (`REQUIRES_HUMAN_INTERVENTION`) repo's phase to `PENDING` through the audited `OPERATOR_REOPEN` door (§12.14, ADR-0125): writes a `findings` row of kind `OperatorReopened` with the required reason. Performs only the reopen — it does not itself re-run any work or clear any dependent's `blocked_by`; a subsequent `fleet resume` does both. Does not reset the phase's `attempts` count (ADR-0125 addendum): a repo already at `max_attempts` re-escalates to `REQUIRES_HUMAN_INTERVENTION` on its very next phase failure. | `<repo>` `--reason TEXT` (required) `--dry-run` |
...
| `fleet resume` | Reconcile after a crash: verify config/version, reap stale worktrees and containers, reset stale `RUNNING` rows, **ask Git whether each ambiguous task's commit landed and correct the row to match** (Git wins; discard the worktree when it did not — §11.5 step 4), demote each repo to its re-entry floor on durable evidence (§11.5 step 5), never a `preconditions_hold` re-check, recompute `blocked_by`, **re-run `stub_reconcile`** (§3.5.1: re-derive every `stubs` row's state from `repos`/`phases`/PR state and re-enqueue any revalidation round lost to the crash under its `revalidation_key` — idempotent, so a resume never doubles the rework), regenerate `migration_state.json`, continue from each repo's re-entry floor (§11.5 step 5), never the earliest incomplete phase — **except a repo whose floor is `SCAN`, which is reported and skipped, never served** (ADR-0080: `fleet scan` takes five flags a resume does not carry and no `wave` at all, so serving that floor means choosing five values the operator never wrote; the skipped repos are named under the payload key `scan_floor_not_continued`, and on a fleet of un-started repos that is most of them). Reads nothing from the model's memory and nothing from `migration_state.json` (§11.5). | `--from-phase 1..4` `--repo NAME` `--reset-attempts` (explicit, audited) `--accept-drift SECTION` (repeatable) `--force-config-drift` `--raise-budget USD` (audited) `--raise-wave-budget USD` (audited) `--repoll-prs` `--revalidation eager\|batched\|manual` `--raise-revalidation-rounds N` (audited) `--dry-run` `--no-continue` |
...
**[Marker 2026-08-28, round-K lane W4 — `D86`: unbuilt. No `initializer=` exists anywhere in `src/` or `tests/`, so the paragraph above describes a mechanism the tree does not have. The hazard it names is real but unreached: the pool's only submitted callable is `scan_file` (`src/fleet/workers/symbolindex.py:266-268`), which resolves no adapter. See §12.47's marker.]**
...
Six ceilings, each with a defined breach behaviour:
...
| `run_max_llm_calls` (unset by default = no cap) | fleet | Run stops dispatching, `RunCallBudgetExhausted`, exit **3**. Sticky without `halted = 1`: the CAS predicate `calls_made + 1 <= max_calls` keeps refusing across every process and restart on its own, so no halt flag is needed and none is set — `--raise-budget` raises the DOLLAR ceiling and would clear a halt without clearing this condition. Raising `budgets.run_max_llm_calls` is the coherent remedy. **Added 2026-09-16 (ADR-0142)**: a sixth ceiling, not a re-wording of the fifth. It exists because `price: free` (§9 rule 5) holds `spent_usd` at `0.00` for the life of a run, which makes every dollar ceiling above structurally inert on a locally-served target — counting dispatches is the only bound such a run can have |
...
- `workers/prwriter.py::_compose()` redacts the PR **title** (before the `[:120]` truncation, so a
  ...
  the only other call site reaching the forge (**corrected 2026-09-15, D142** — this read
...
- `llm/calls.py::render_prompt()` redacts the evidence mapping (`redact_mapping()`) before it is
  ...
  (**corrected 2026-09-15, D142** — this read `llm/client.py` for both; `client.py` calls no
...
`attempts`**. Nothing came back ⇒ nothing landed: `git reset --hard <tasks.pre_commit_sha> &&
git clean -fdx` — **the TASK anchor, never the phase one**: step 6.5 above forbids rewinding a
crashed task to `phases.base_ref` (it would delete the commits of earlier tasks in the same phase
whose rows are already `DONE` and will never re-run), and `vcs/commits.discard_task` takes
`task_pre_commit_sha` precisely so the other one cannot be expressed —
...
tree-SHA comparison, because a commit is either on the branch or it is not (§3.2 step 6) — **a claim about Git's *verdict*, and not about a question Git could not be put.** A candidate whose worktree is gone, whose branch or anchor does not resolve, or whose probe did not settle is reported unreconciled with its reason and its row left byte-for-byte as it was; folding that into either branch would mark a task `DONE` carrying a SHA nobody found, or `reset --hard` a tree that may hold the only copy of the work (ADR-0087 §3); **Marker 2026-09-01 (D89 Phase 2 Task B, fix wave):** this "no third branch" claim is per-`task_id`, i.e. still exactly true for a single Git question asked about one identity. It is no longer the whole picture for the REWRITE kind: since D89 Phase 2 Task B, a REWRITE coarse row asks Git about each of its per-unit synthetic ids individually (ADR-0102's coarse-row identity, ADR-0103's per-unit reconciliation), and a row whose units land only partially now gets a genuine third verdict — "partially landed" — carried in `report["partially_landed"]` and rendered by `cli._arbitration_lines`. Decision recorded in ADR-0103; this sentence is left as originally written per this project's "annotate, never rewrite" convention;
...
2. **Static gates.** `ruff check src/ tests/`, `ruff format --check src/ tests/`, and `mypy --strict src/fleet/` all exit 0. **Adjudicated 2026-09-03 (round VI controller, ADR-0116):** the `ruff format --check` leg's literal "exit 0" is relaxed to a pinned dirty-count baseline (116 dirty in this criterion's own `src/ tests/` scope, 123 whole-repo, both re-measured fresh — `tests/test_lint_gate.py::test_ruff_format_check_dirty_count_matches_the_pinned_baseline`), which fails loudly on any new drift in either direction. Reformatting the pre-existing dirty files as a side effect of this criterion is explicitly out of scope. See ADR-0116.
...
6. **The invariant holds** (§1, ADR-0020). Language knowledge exists **only** in `src/fleet/manifests/`, `src/fleet/ecosystems/`, and `config/rules/`. Enforced by two mechanical gates, both of which must exit 0: (a) an AST test that walks **every** module under `src/fleet/` except those two packages and fails on any `Compare`, `match`, or subscript node whose operand is an `Ecosystem` or `ContractKind` member — the exemption list is exactly two package paths and is asserted to be exactly two, so widening it is a visible diff; **scoped 2026-08-31 (round Q controller, ADR-0100): a `Subscript` node is only a violation of this clause when its base is not a name bound to a dict/mapping literal declared at module scope in the same file — a registry-table read (e.g. `SYMBOL_IDENTIFIED[ContractKind.PROTO]`) is the compliant "table, not branch" pattern this criterion requires, not an instance of the branching it forbids; dict-literal key definitions were never `Subscript` nodes and needed no scoping.** (b) `grep -rnE 'if +(unit\.|repo\.|coordinate\.)?ecosystem *(==|!=|is)|Ecosystem\.[A-Z]' src/fleet/ --include='*.py' | grep -v '^src/fleet/\(manifests\|ecosystems\)/' | grep -v '^src/fleet/models/enums.py'` returns nothing. `src/fleet/bazel/` carries **no** exemption: `bazel/layout.py`, `bazel/generators.py`, `bazel/lockfile.py` and `bazel/query.py` — every module in the package, `generators.py` explicitly included (ADR-0065) — are drivers and pure renderers, and a per-ecosystem branch appearing in any of them is precisely the regression this criterion exists to catch. Gate (a) ships in `tests/test_ecosystems.py` as four line-scanning tests rather than one AST walk (`test_no_ecosystem_branch_exists_outside_the_adapter_packages`, `test_no_ecosystem_member_other_than_the_unknown_sentinel_is_named_outside_the_packages`, `test_the_exemption_list_is_exactly_the_two_adapter_packages`, `test_no_language_directory_is_hardcoded_in_any_driver`), plus a fifth — `test_no_adapter_package_exception_is_named_outside_the_adapter_packages` — covering the case no grep can see: an adapter-private `…Error` class named by a driver's `except` clause.
7. **Manifest adapter completeness.** All six shipped `ManifestAdapter`s parse manifests synthesized inline via `tests/test_manifests.py`'s `write(tmp_path, name, body)` helper and produce `Coordinate`s whose `key` matches the fixture's expected value. **Adjudicated 2026-08-30 (round M controller, ADR-0099), resolving the adjudication round-K's `12be741` marker left open:** committed fixture repos under `tests/fixtures/repos/` are not built to satisfy this sentence — the property already holds via inline `tmp_path` synthesis, a self-contained approach this codebase's test suite already uses successfully elsewhere, and standing up a parallel committed-fixture-repo mechanism would be pure churn for no functional gain (`CLAUDE.md` Rule 2). The criterion is reworded above to name the actual mechanism.
...
11. **Phase 3 exit condition** (§3.3): `bazel build //<dest>/...` and `bazel test //<dest>/...` both exit 0 inside a `--network=none` container, with each invocation recorded as an `attempts` row carrying its exit code. **And the tests survived the move**: for every repo whose `repos.baseline_ok = 1`, `bazel query 'tests(//<dest>/...)' | wc -l` is `>= repos.baseline_test_count`, asserted per repo and reported as a table of `(repo, baseline, migrated)` triples. Green-and-empty is the failure this catches — a `filegroup` emitted where a `*_test` should have been builds and "tests" clean at zero targets, which passes the first half of this criterion while deleting the repo's entire safety net. Repos with `baseline_ok IS NULL` (baseline never measured, e.g. `preflight.baseline_build.enabled: false` or a pre-schema-7 run) are excluded from the count assertion, not silently passed: the fixture run asserts the exclusion set is empty under the shipped config. **Narrowed 2026-09-09 (round VI controller, ADR-0135), resolving one of D116's three judgment calls, same shape as §12.9(a)'s 2026-08-30 correction above:** "the exclusion set is empty" is unsatisfiable as written regardless of mechanism quality — an `EmptyRepo` in the fixture fleet never gets a worktree cut (`workers/clone.py`), so no native baseline build can ever run against it, structurally, not as a bug. Corrected to read: *the exclusion set contains only repos matching one of §3.1 criterion `(c)`'s five enumerated exemptions* — the same narrowing §12.9(a) already applies to its own manifest-coverage assertion, for the identical underlying reason. Do not write a fabricated `baseline_ok=0` for an exempt repo to force the unnarrowed sentence true — see ADR-0135 for why that would corrupt §12.9's own exemption-closure fixture. The landing task (a future Leg C/E) still owes confirming the exact applicable exemption subset against the live fixture fleet.
...
15. **Crash safety, and Git is the arbiter** (ADR-0024). `SIGKILL` at any point during a fixture run, followed by `fleet resume`, re-derives `migration_state.json` from SQLite, re-runs no `SUCCEEDED` phase, re-attempts no abandoned repo, and leaves zero orphaned worktrees or containers. The two hard cases are induced deterministically and asserted separately. (i) **`SIGKILL` between `git apply` and `git commit`:** the worktree is dirty and no commit carries the task's `Fleet-Task-Id`, so resume discards the worktree onto **`tasks.pre_commit_sha`** — this task's own anchor, never `phases.base_ref`, which would delete the commits of earlier `DONE` tasks in the same phase (§3.2 step 6.5; `vcs/commits.discard_task`) — re-runs the rung, and the branch ends with **exactly one** commit for that `Fleet-Patch-Id`; `phases.attempts` is **unchanged** across the crash. (ii) **`SIGKILL` between `git commit` and the SQLite write** — the case a naive harness re-applies and corrupts: the commit exists on `migrate/<repo>`, `attempts.commit_sha` is NULL, and resume corrects the *row* from Git, re-running nothing and adding no second commit. Fabricating the reverse disagreement — hand-editing `attempts.commit_sha` to a SHA that is not on the branch — makes resume correct the column, never `git reset` the branch to match it, which is the mechanical form of the "Git wins" invariant (§11.5).
16. **Checkpoint integrity.** A byte-corrupted `checkpoints.payload` is **invalidated, never raised** — `state/checkpoints.py::load` returns `payload is None` carrying a `CheckpointRejection` (`MALFORMED_ENVELOPE` for a corrupt envelope, `INVALID_PAYLOAD` for data that no longer validates) — and causes the step to re-run, never a partially-populated object (Constraint 2). **Corrected 2026-08-27 (audit of §12 against the test suite, base `076076d`):** this criterion read "raises `ValidationError` on load" until then, and it never held: `load()` signals every failure by returning a `CheckpointRejection`, and no member of that enum propagates an exception, while `tests/test_checkpoints.py::test_a_truncated_blob_invalidates_instead_of_raising` asserts the opposite **in its own name**. A reconciler must **not** make `load()` raise to satisfy the retired sentence — the never-raise property is deliberate, and that module's own docstring states it (`src/fleet/state/checkpoints.py:8-15`). Cited rather than quoted, because nothing enforces a copy of another module's comment (CLAUDE.md §3). §12.48's cross-reference was corrected in the same change. **Corrected 2026-08-28 (round-K repair of `12be741`, lane W4):** the 2026-08-27 marker attributed the words “*must never raise and never half-populate a model*” to `state/checkpoints.py`'s docstring. **They are not in that file.** A whitespace-normalised sweep of `src/` and `tests/` at `12be741` finds the phrase exactly **once**, in the module docstring of `tests/test_checkpoints.py` (wrapped across `:4-5`, which is why a line-oriented `grep` reports zero) — the test module, not the module under test. The quote is replaced by the citation above; the retired words are kept here so a sweep for them finds this correction and not a survival.
17. **Projection fidelity.** For any run state, `migration_state.json` regenerated from SQLite is byte-identical to the file on disk, and the file is always valid JSON (verified by killing the process mid-write in a test). **Disclosed 2026-08-28 (round-K repair of `12be741`, lane W4):** **the byte-identity half is FALSE BY CONSTRUCTION and is not a passing gate.** `MigrationState.updated_at` is `Field(default_factory=utcnow)` (`src/fleet/models/state.py:236`) and `state/projection.py::build_state` does not supply that key (`:213-238`), so two projections of an **untouched** database differ in it — the same quantity CLAUDE.md's Guardrail 6 records defeating a round-F detector that hashed the whole file. The JSON-validity half is unaffected. A reconciler must **not** delete the `default_factory` to make the sentence true: a projection with no write timestamp is a worse artefact than a non-reproducible byte string. Choosing the satisfiable replacement — byte-identity over the dump with `updated_at` excluded, or an equality over the state vector — is an **adjudication** this marker does not make. Listed in the 2026-08-27 audit's unsatisfiable class and left neither corrected nor disclosed until now. **Corrected 2026-09-01 (round Y task 1, ADR-0106):** fixed via the first option this marker priced — `state/projection.py::build_state` now derives `MigrationState.updated_at` deterministically from the latest of the already-read `phases.updated_at` / `waves.computed_at` / `contracts.detected_at` / `collisions.detected_at` timestamps (falling back to `runs.started_at` for a run with none of those rows yet), instead of leaving the key unsupplied and letting `Field(default_factory=utcnow)` fire at build time. This is not the forbidden "delete the default_factory" path: the field still carries a meaningful last-write instant for a human reading the file — it is now sourced from durable SQLite state instead of wall-clock-at-build-time, so two builds of an unchanged database agree and a real write moves it, same as every other field in the projection. The byte-identity half is a passing gate again; `tests/test_projection.py::test_two_projections_of_an_untouched_database_are_byte_identical` proves it directly on raw file bytes. See ADR-0106.
...
19. **Cycles are broken, not merely detected.** A planted 3-repo cycle **with no shared contract in it** produces a `CycleFinding` with `break_strategy = EDGE_BREAK`, a non-empty `broken_edge_ids`, and an acyclic ordering subgraph; a planted 12-repo cycle **likewise contract-free** produces `break_strategy = ATOMIC_WAVE` with all 12 members sharing one `wave_index` and one `PullRequestDraft.scc_id`; a planted 41-repo cycle produces `break_strategy = MANUAL` with all members `REQUIRES_HUMAN_INTERVENTION` and the rest of the fixture fleet still completing. In every case `nx.is_directed_acyclic_graph` holds over the final ordering subgraph, and the run terminates — no hang. **Adjudicated 2026-09-03 (round VI controller, ADR-0114), `PullRequestDraft.scc_id` clause only:** the literal text above asks for the shared-PR proof specifically at 12-repo scale. What is built is a real 2-repo `fleet pr` proof of the wiring (`tests/test_pr_e2e.py::test_an_atomic_wave_scc_ships_one_pr_shared_by_every_member`) plus the pre-existing, independent 12-repo proof of the underlying SCC computation (`tests/test_graph_cycles.py::test_a_12_repo_cycle_shares_one_scc_id_across_all_members`) — adjudicated as jointly satisfying this clause's intent in place of one combined 12-repo `fleet pr` fixture: the new wiring is confirmed N-generic (no branch anywhere depends on `len(scc.members)`), so a 12-repo `fleet pr` run would exercise the identical code path N times over an SCC set already proven correct at N=12 by an independent test, catching no defect class the two existing proofs miss between them. The EDGE_BREAK (3-repo) and MANUAL (41-repo) clauses are unaffected by this adjudication. See ADR-0114.
20. **Secrets never leak.** After a fixture run whose repos contain a token in the mirror's remote URL, a token echoed by a build script, and a tracked `.env`: `grep -rIE 'github_pat_|ghp_|xox[baprs]-|AKIA[0-9A-Z]{16}|sk-ant-|sk-[A-Za-z0-9]{32,}|"private_key_id"|://[^/[:space:]:@]+:[^/[:space:]@]+@' logs/ artifacts/ migration_state.json` returns nothing; the same grep over `SELECT payload FROM events`, `SELECT stderr_tail FROM attempts`, `SELECT last_error FROM phases`, and `SELECT response_json FROM llm_cache` returns nothing; and the generated PR body contains the `«redacted:…»` placeholder rather than the value. **Adjudicated 2026-09-01 (round V controller, ADR-0104):** the literal text above describes one combined fixture run planting all three secret shapes simultaneously and grepping across the log/artifact/state paths, the four named DB columns, and the PR body in one pass. Per-property coverage — each of the four DB columns (`events.payload`, `attempts.stdout_tail`/`stderr_tail`, `phases.last_error`, `llm_cache.response_json`) and the PR-body placeholder independently proven through its own real write path, each with a mutation-discriminated redaction assertion and an over-redaction control — is adjudicated as satisfying this criterion's intent in place of the literal one-fixture methodology: the underlying property (a planted secret never survives to the persisted/rendered artefact) is proven for every named site either way, and a combined fixture would exercise the same properties simultaneously rather than independently, catching no defect class the per-property tests miss. See ADR-0104.
...
27. **Collisions are caught before transformation.** Two fixture repos publishing the same coordinate produce a `collisions` row of kind `COORDINATE` during `fleet sequence`, at `severity='warn'` and carrying the §13 row 6 ownership ladder's verdict as its `resolution`; and an unresolved `severity='error'` row makes `fleet sequence` exit 6, with that row already durable in `collisions` when the process refuses. The only blocking row `fleet sequence` can produce is a `COORDINATE` whose carriers' `manifests.ecosystem` values disagree, so the exit-6 half is exercised by injecting that one column into an otherwise real, fully scanned database — everything downstream of it (claim assembly, detector, upsert, gate, exit code) is the real thing. **[Corrected 2026-08-28, round-L lane W8, base `a13a971`; `src/fleet/cli.py` line numbers re-derived against `a8f52c9` by round-L lane W9, `cli.py` having gained 5 lines at `:1759` since the base — each re-derived by matching the cited text, not by adding 5.** The retired words, quoted so a sweep for them finds this correction and not a survival: *“two computing the same `dest_path`, and two shipping a byte-identical `LICENSE` produce `collisions` rows of kinds `COORDINATE`, `DEST_PATH`, and `FILE_PATH` during `fleet sequence`; the identical `LICENSE` is deduped rather than suffixed”*. **The exit-6 requirement itself is NOT retired** — it is real, and round K gave it a second enforcement point in `src/fleet/cli.py:2972-2979`, raised after `StateWriter.submit` has awaited its transaction so the operator is left a row to act on. What is retired is the *examples*, on two independent measurements at `a13a971`. **(a) `DEST_PATH` and `FILE_PATH` rows cannot be produced by any command, let alone by `fleet sequence`.** `audit_collisions` has exactly two callers in `src/`: `cli.py:2952-2957`, which constructs `CollisionInput(coordinates=…, owns_hints=…)`, and `src/fleet/workers/contracts.py:372-379`, which passes `contracts=` only. `dests`, `files` and `versions` default to `()` (`src/fleet/graph/collisions.py:128-130`), so `_dest_collisions`, `_file_collisions` and `_version_collisions` run over empty input everywhere. **(b) None of the three named scenarios could satisfy the exit-6 clause even if the rows existed.** `CollisionReport.blocking` is `severity == 'error' and resolution is None` (`collisions.py:155-158`), and exactly two of the five detectors have a `resolution=None` branch: `_coordinate_collisions` when the carriers' ecosystems diverge (`:246-247`) and `_version_collisions` on an empty version intersection (`:468-475`, unreachable per (a)). `_contract_collisions` (`:284`), `_dest_collisions` (`:358-365`) and `_file_collisions` (`:395-422`) set a resolution on every branch and therefore can never block — two repos computing one `dest_path` and two shipping a byte-identical `LICENSE` are warn-and-resolved by construction. A reconciler must **not** close the gap by passing `dests=`/`files=` at the `fleet sequence` call: `tests/test_collisions_wiring.py:11-22` records why both are deliberately unwired — nothing captures a per-file path/blob-SHA listing and no table holds one, so `_file_collisions`' `len(shas) <= 1` dedupe branch would fire for every group including genuinely divergent files, and `dest_rewrites` has no consumer, so persisting a `DEST_PATH` row would assert a relocation the harness does not perform. That the divergent-ecosystem fault cannot arise from a fixture and must be injected is that file's own measured finding (`tests/test_collisions_wiring.py:365-389`), relayed here rather than re-derived.]**
28. **Single writer.** An integration test that starts the runner and attempts a second writable `aiosqlite` connection in the same process raises; process-pool children are constructed with no database handle (asserted by inspecting the initializer arguments); and a 200-repo simulated run produces zero `SQLITE_BUSY` errors. **Disclosed 2026-08-28 (round-K repair of `12be741`, lane W4):** **this clause is NOT the `D86` clause, and must not be retired with it.** `D86`'s heading at `12be741` said §12.28 and §12.47 *both* require an initializer that calls each `discover()`. §12.28 does not: it requires only that a pool child carry **no database handle**, which is a weaker and cheaper requirement and is **true in the tree today** — the sole `ProcessPoolExecutor(` construction (`src/fleet/orchestrator/budgets.py:947-949`) passes `max_workers=` and `mp_context=` and nothing else, so no handle can cross. Only §12.47 carried the `discover()` claim, and only §12.47's copy is retired. See `D86` in `docs/INTEGRATION_HONESTY.md`.
29. **Contracts are extracted once, deterministically, and the graph stays whole.** Three fixture repos vendoring a byte-identical `identity.proto` plus two carrying only its generated `*_pb2.py` produce exactly **one** `contracts` row (`proto:acme.identity.v1`), one `owning_repo_id` chosen by the §3.1 5b (iv) ladder, ≥2 `consumer_repo_ids`, one `collisions` row of kind `CONTRACT` at `severity='warn'`, and zero repos proposed as owner on the strength of generated code. `extraction_confidence` is reconstructible — `0.9 × Π confidence_factors.values()` equals it to within 1e-9 — and a divergent fourth copy flips the collision to `severity='error'` and applies the `divergent` ×0.5 modifier. **Adjudicated 2026-09-03 (round VI controller, ADR-0117):** the divergent-fourth-copy clause above is confirmed structurally unreachable from a real `fleet scan` today — `cli.py::_extract_contracts` constructs `ContractsInput` with no `blob_shas=` argument, so `content_sha256` is always empty and neither the severity escalation nor the modifier can fire from real scan data, the same root cause as the disclosed §12.9/§12.27 blob-SHA capture blocker (no D-number). A worker-level proof against `ContractsInput.blob_shas` — a real, documented input field the worker fully honours — (`tests/test_contracts_criterion_scale.py::test_a_divergent_fourth_copy_flips_the_collision_and_applies_the_modifier`) is adjudicated as satisfying this clause's intent in place of literal end-to-end real-scan reachability, mirroring ADR-0065's precedent for §12.32's equivalent situation. See ADR-0117. Node integrity holds: every `edges.src_id`/`dst_id` and every `wave_members.node_id` resolves in `repos` or `contracts` according to its sibling `*_kind` (asserted by a query returning zero rows, since no FK enforces it).
...
31. **A wrong extraction is detected and rolled back before it costs three attempts.** Two induced failures on the fixture fleet: (i) a contract whose consumers all resolve to the owner produces a `ContractNotShared` finding, `contracts.status='REJECTED'`, and no contract node in `wave_members`; (ii) a hoist whose contract wave fails `bazel build` produces a `HoistBrokeOwner` finding, `contracts.status='FAILED'`, a `git revert -m 1` of the contract merge carrying the standard `Fleet-*` trailers (found by `git log --grep='^Fleet-Task-Id: '` on the integration branch, not by a SQLite row), every affected edge un-hoisted — its contract-kind row excluded from the next graph build and the untouched pre-hoist repo→repo row standing in its place, asserted row-by-row against a pre-hoist snapshot — a re-sequenced SCC that falls through to `EDGE_BREAK` or `ATOMIC_WAVE`, `phases.attempts` **unchanged** for every SCC member, and a run that still completes the rest of the fleet. **Corrected 2026-09-06 (round VI, D111 Leg D design pass, ADR-0122):** this clause read "every affected edge restored from `retargeted_from_repo_id` (asserted row-by-row against a pre-hoist snapshot)" until then — `D117` (`docs/INTEGRATION_HONESTY.md`) found that reconstruction structurally impossible for a REPO-dst edge, and ADR-0122 is the disclosed Rule-14 adjudication choosing the real mechanism (exclusion at graph-build read time, never reconstruction, never a `DELETE`). The **observable proof bar is unchanged** — still a row-by-row assertion against a pre-hoist snapshot — only the sentence's description of *how* that observable is achieved changes, from a false mechanism to the real one. **Adjudication pending, 2026-09-08 (round VI, controller ruling preceding task 95's dispatch, carried out by task 96 — `.superpowers/sdd/round-VI-criteria-closure/progress.md`):** the literal trigger this clause names — "a hoist whose **contract wave** fails `bazel build`" — is architecturally unreachable under this project's current wave-dispatch design, which filters every dispatch-side wave reader to `node_kind='REPO'` (ADR-0119's own deliberate safety scoping; a contract-kind wave is computed and persisted in `wave_members` but never itself dispatched). Building contract-wave dispatch to manufacture this literal trigger would contradict that scoping and is **not decided here** — it is a genuine open question for a future round, hence **adjudication pending** rather than a Rule-14 rewording. **Resolved 2026-09-09 (round VI controller, ADR-0134): narrowed, not built.** Reversing ADR-0119's deliberate safety scoping to manufacture the literal contract-wave trigger is rejected — the substituted trigger below already proves every other observable clause equivalently. This clause's trigger is therefore formally narrowed (Rule 14) from "a hoist whose contract wave fails `bazel build`" to "a hoist whose contract wave is discovered wrong when a REPO-kind consumer wave downstream of it fails `bazel build`" — see ADR-0134 for the full decision and rejected alternative. Pending that decision, the closing task's proof target is the closest reachable analogous trigger: a REPO-kind **consumer** wave failing downstream after receiving a bad hoist merge — the wrong-hoist is discovered when the *consuming* wave fails, not a never-dispatched contract wave. Every OTHER clause of case (ii) — the `HoistBrokeOwner` finding, `contracts.status='FAILED'`, the `git revert -m 1` of the contract merge (now anchored on the `Hoisted-Contract:` trailer, never a SQLite row — `vcs.commits.find_contract_hoist_merge`, round VI task 96), and the edge un-hoisting — is proven under this substituted trigger, and the observable text for those clauses is otherwise unchanged. See `docs/CRITERIA_PLAN.md` §31 and `docs/INTEGRATION_HONESTY.md` D111 for the full breakdown of what remains open (this trigger question, and the untested "falls through to `EDGE_BREAK`/`ATOMIC_WAVE`" clause below). **Corrected 2026-09-09 (round VI task 101, TEST-ONLY):** the "falls through to `EDGE_BREAK`/`ATOMIC_WAVE`" clause named just above is no longer untested — `tests/test_graph_cycles.py::test_a_rolled_back_hoist_re_sequences_to_edge_break` and `::test_a_rolled_back_hoist_re_sequences_to_atomic_wave` each drive two real `break_cycles` calls (hoist for real, flip the same contract to `FAILED`, re-sequence) and assert the SCC falls through to the named strategy; the mechanism was verified correct by construction, not defective, so no production code changed. Only the literal trigger-unreachability adjudication (this same paragraph's opening sentence) remains open for this criterion's case (ii); see `docs/CRITERIA_PLAN.md` §31's task-101 update for the precise breakdown, including the still-untested `phases.attempts`-unchanged sub-clause task 96 separately disclosed, which task 101 was not scoped to and did not close.
32. **The adapter registries are total, and delegation is honest** (ADR-0020). `fleet.ecosystems.discover()` returns a mapping whose key set equals `set(Ecosystem)` **exactly** — a test that adds a decoy `Ecosystem` member with no adapter makes `discover()` raise at import time, and a decoy adapter claiming an already-claimed `Ecosystem` raises `RuntimeError` from `@register` rather than overwriting. The second half of this criterion — `fleet.ecosystems.contracts.discover()` satisfies the same equality against `set(ContractKind)` — is **UNSATISFIABLE AS WRITTEN and is NOT a passing gate** (ADR-0065): `src/fleet/ecosystems/contracts/` does not exist, so there is no `discover()` to call. It remains the target shape; until that package lands, the standing obligation is the weaker one `workers/contracts.py` already meets — every `ContractKind`-shaped fact is a `Mapping[ContractKind, …]` table and not a branch, which §12.6 does enforce. For every registered `EcosystemAdapter`, `generate_targets(unit) == []` **if and only if** `uses_gazelle` is true and `gazelle_config(unit) is not None` — so no adapter can claim to delegate and also emit, or claim to emit and return nothing. [2026-09-06, round VI research-33/controller — **superseded by the adjudication below; kept per the annotate-never-rewrite discipline.** The prior marker read: "this carve-out's own premise … is **adjudication pending**, not decided here."]

[**Adjudicated 2026-09-06, round VI research-34.** The carve-out above is **WITHDRAWN**, not merely stale: `src/fleet/ecosystems/contracts/` exists (`base.py` + five adapters, round VI task 40, `f3c0200`) and `fleet.ecosystems.contracts.discover()` is callable, so the second half of this criterion is **satisfiable and is a passing gate from this date forward**. It is *not* waived and its bar is *not* relaxed — the obligation is exactly the one the sentence already states, the **key-set equality against `set(ContractKind)`**; the decoy-member and duplicate-claim sentences above are written about `Ecosystem` and are **not** extended to `ContractKind` by this adjudication. The equality is measured TRUE at `6844a04` by two independent derivations, but at that commit **no test asserts it**, so §12.32 returns to OPEN — TEST-ONLY until one lands; see `docs/CRITERIA_PLAN.md`'s §32 entry. Full measurement: `.superpowers/sdd/round-VI-criteria-closure/research-34-report.md`.]
...
40. **No model string exists outside configuration** (ADR-0023). `grep -rnE '\b(claude|gpt|gemini|llama|mistral|qwen|deepseek|phi|mixtral)[-_.][A-Za-z0-9]' src/ --include='*.py' | grep -v '^src/fleet/llm/backends/'` returns nothing, and so does `grep -rnE 'https?://[^"'"'"'[:space:]]*/v1' src/ --include='*.py' | grep -v '^src/fleet/llm/backends/'` — no model id and no endpoint URL is written in Python. A stricter structural form backs the grep up: `grep -rnE '^\s*(import|from)\s+(anthropic|openai|boto3|google\.)' src/fleet/ --include='*.py' | grep -v '^src/fleet/llm/backends/'` returns nothing, so no vendor SDK is importable outside a backend file, and an AST test asserts that the only module in `src/fleet/` reading `config/models.yaml` is `settings.py`. The one place model ids legitimately appear — `config/models.yaml` — is asserted to parse into a `ModelTier`-keyed profile map for every shipped profile. **Corrected 2026-08-27 (audit of §12 against the test suite, base `076076d`):** the AST clause named `llm/routing.py`, which does not exist and never has — the reader is `settings.py` (`models_path = cfg_dir / "models.yaml"`); `docs/PROGRESS.md` already recorded "none exists, none should be created", so this is a rename, not a new decision. **Both greps as literally written also returned hits at `076076d` rather than nothing**: the model-id grep returned 7, every one the product name `llama.cpp` in a comment or docstring inside `llm/backends/openai_compatible.py` (the `.c` of `.cpp` satisfies `llama[-_.][A-Za-z0-9]`), and the endpoint grep returned 1, the URL *template* at `llm/backends/vertex.py` whose host comes from config. Both live under `llm/backends/`, which is precisely where §12.40 permits vendor detail — so **both greps take the same `| grep -v '^src/fleet/llm/backends/'` exclusion the SDK-import form already carries**, and only then are they gates. The structural form that actually enforces the intent returns **0** unscoped. Do **not** delete the `llama.cpp` prose or inline the Vertex host to make the unscoped greps pass. **Corrected 2026-08-28 (round-K repair of `12be741`, lane W4):** *the 2026-08-27 marker asserted an edit it did not make, and inverted one of its own numbers. Both retired sentences are quoted here so a count-based sweep finds this correction rather than a survival.* **(a)** “*both greps take the same `| grep -v '^src/fleet/llm/backends/'` exclusion the SDK-import form already carries*” was **false when written**: at `12be741` the exclusion appeared on this line only inside the SDK-import command and inside that sentence, never on the two greps it describes, so both still returned hits (7 and 1, re-measured at `12be741`) and §12.40 stayed unsatisfiable while the same marker forbade the only repairs a reader would reach for. The exclusion is **applied to both greps in this commit**, which is what makes the sentence true; with it, both return **0**. **(b)** “*The structural form that actually enforces the intent returns **0** unscoped*” is **inverted**. Measured at `12be741`: the SDK-import form **without** the exclusion returns **5** — `llm/backends/anthropic.py:24`, `bedrock.py:46`, `openai_compatible.py:44`, `vertex.py:58` and `:59` — and **0** only **with** it. The predicate matters more than the number: *no vendor SDK import outside `llm/backends/`* is the class this form enforces, and that class held at `12be741` and holds now. **(c) A contradicting record was resolved silently (CLAUDE.md Rule 7).** The 2026-08-27 marker cited `docs/PROGRESS.md:5542` (“none exists, none should be created”) and did not mention `docs/PROGRESS.md:777`, which addresses the same clause and resolves it the **other** way — “`llm/roles.py` holds the router that §8 and §12.40's AST test expect at `llm/routing.py`** — a rename plus import update”. `settings.py` wins because the clause’s predicate is *reads `config/models.yaml`*, not *routes roles*: `settings.py` opens the file (`:1138` `models_path = cfg_dir / "models.yaml"`, `:1149` `_read_yaml_mapping(models_path)`), while `roles.py` is documented and built over an **already-parsed** tree (`:112`, `:161`). Class result at `12be741`, re-derived here: of the modules under `src/fleet/` that name `models.yaml`, exactly **one** opens it, and it is `settings.py`. **(d)** The AST clause is a **statement of intent, not of coverage**: a whitespace-normalised sweep of `tests/` for an assertion that `settings.py` is the sole reader of `config/models.yaml` returns **zero** at `12be741`. No such test exists; the two `models.yaml`-and-§12.40 hits in `tests/` (`test_llm_backend_vertex.py:755`, `test_llm_backend_bedrock.py:783`) assert the model-id half instead. **(e) What the exclusion costs, stated rather than left to be discovered.** Adding it was validated as a gate, not assumed: on the clean tree both greps read **0**; with a model id and a `/v1` endpoint injected into `src/fleet/llm/client.py` — outside `llm/backends/` — both read **1**; the injection was reverted and `git status src/` is clean. The blind spot the exclusion buys is exact and is the reason the structural SDK-import form is kept beside these two: **a hard-coded model id or endpoint inside a `llm/backends/` file is now invisible to both greps.** That is the same trade §12.40 already made for the SDK-import form, and it is why the backends' own tests (`test_llm_backend_vertex.py:755`, `test_llm_backend_bedrock.py:783`) assert the model id comes from config. Alternatives were rejected by measurement, not by argument: dropping the exclusion leaves the criterion unsatisfiable (7 and 1), and narrowing the pattern to spare `llama.cpp` and the Vertex template is the “delete the prose / inline the host” repair this marker already forbids. See `D86`’s neighbours in `docs/INTEGRATION_HONESTY.md` and the residue section of `docs/superpowers/plans/spec12-success-criteria-audit.md`.
41. **A local-only profile runs the whole pipeline** (ADR-0023). With `--profile local` pointed at a stub OpenAI-compatible server started by the test (no external network reachable: the test asserts every outbound connection targets `127.0.0.1`), a full fixture run completes `scan → sequence → transform → build → verify → pr` with the same exit code and the same set of `SUCCEEDED` repos as the `default` profile on recorded fixtures. Three specifics make it a real proof rather than a smoke test: the `CHEAP` targets declare `supports_json_schema: false` and `supports_tools: false`, so every classification call is genuinely negotiated down to `PROMPTED` and still yields Pydantic-valid results (`LlmCallRecord.structured_output_mode == 'PROMPTED'` for those rows); `ANTHROPIC_API_KEY` is **absent** from the environment for the entire run; and `SELECT DISTINCT backend FROM llm_cache` for the run's rows equals `{'openai_compatible'}`. Switching between the two profiles requires **zero** source edits (`git diff --stat src/` is empty), which is the mechanical form of the ADR-0023 claim. **Adjudicated 2026-09-01 (round W controller, ADR-0105):** the literal text above says the `CHEAP` targets "declare" both fields `false`; `config/models.yaml`'s actual `CHEAP` entry omits both keys from `capabilities_override` entirely, deliberately (see its own comment). Traced through `merge_capabilities`: an omitted override key falls through to `openai_compatible.declared_capabilities()`'s bare-default floor, which is already `False` for both fields — the merged result is byte-identical to an explicit `false` declaration. This is a wording-precision correction, not a behavior change: the criterion's substantive claim (CHEAP genuinely lacks structured-output capability at negotiation time, forcing `PROMPTED`) holds exactly as stated. See ADR-0105.
...
45. **No code state is persisted outside Git** (ADR-0024) — the structural assertion that the shadow-VCS anti-pattern cannot creep back. Three checks, all mechanical. (i) **Schema:** `PRAGMA table_list` contains no `mutations` table, and across every table `PRAGMA table_info` yields no column whose name matches `(pre|post)_tree_sha|tree_sha|patch_sha256|patch_path|patch_blob|diff_blob|(^|_)diff(_|$)|rollback_log`; the git-SHA-shaped columns that exist (`repos.head_sha`, `phases.pre_commit_sha` / `post_commit_sha`, `tasks.pre_commit_sha`, `attempts.commit_sha`) each resolve to a real commit on the repo's branch, asserted with `git cat-file -e <sha>^{commit}`. **Corrected 2026-08-27 (audit of §12 against the test suite, base `076076d`):** as first written this check **could not pass**. Its bare `blob` alternative matched two legitimate columns — `repos.largest_blob_bytes` and `collisions.blob_shas` — and its bare `diff` alternative would match any future column merely containing that substring; the shadow-VCS pattern this guards against is a column *holding* a diff or a tree SHA, not one whose name contains "blob". The replacement alternation was validated both ways before landing: it matches **no** column in `state/schema.sql` today, and it still matches all of `pre_tree_sha`, `patch_path`, `rollback_log`, `diff_blob` when those are injected synthetically. The enumeration was also incomplete — it named three of the **five** git-SHA-shaped columns, omitting `repos.head_sha` and `tasks.pre_commit_sha`. Note only `attempts.commit_sha` carries a `length(...) = 40` CHECK (`state/schema.sql`); the other four are conventionally 40-hex but unconstrained, so the `git cat-file` assertion is what binds them. Do **not** rename or drop `blob_shas` / `largest_blob_bytes` to satisfy the retired regex. (ii) **Content:** after a full fixture run, no persisted value anywhere in `state/fleet.db` contains a unified-diff hunk header — `SELECT` across every `TEXT`/`BLOB` column matching `'%@@ -%+%@@%'` returns zero rows — so a diff cannot have been smuggled into a payload, a checkpoint, or an error field. (iii) **Sufficiency, the point of the whole change:** deleting `artifacts/` entirely and re-running `fleet resume` completes the fixture fleet with the same set of `SUCCEEDED` repos and the same `run_digest`, proving `artifacts/diffs/` is an export rather than state; and conversely, restoring a database snapshot taken *before* a phase's commits while leaving the branches intact makes `fleet resume` re-derive that phase's outcome from the `Fleet-Task-Id` trailers and add **no** duplicate commits. Every fixture commit on a `migrate/*` branch is asserted to carry all six `Fleet-*` trailers, parsed with `git interpret-trailers --parse`.
...
| 6 | **Two repos publish the same coordinate** | **Two mechanisms, two commands, neither of them a PK conflict.** During `fleet scan`: `OwnerIndex.owners_of(coord_key)` returns more than one repo. During `fleet sequence`: §3.1 step 8's audit groups `manifests.publishes_key` and finds ≥2 distinct `repo_id`s | **At `fleet scan` — the ambiguity half:** every affected edge is marked `ambiguous = 1` with `dst_candidate_repo_ids` listing both candidates and is ordered against **both** (over-ordering is safe). **At `fleet sequence` — the collision-row half:** one `collisions` row of kind `COORDINATE`, its owner chosen by `owns:` hint (only when the hint names a repo that actually publishes the coordinate) → shallowest declaring path → higher `commit_count` → `repo_id`, recorded as the row's `resolution`; `severity='error'` only when the carriers' ecosystems disagree. The `coordinates` upsert itself is `DO UPDATE … COALESCE`, and it writes no collision row | `edges.ambiguous` / `dst_candidate_repo_ids` (`src/fleet/graph/infer.py:320`, `:326`, from `OwnerIndex.owners`), reached by `fleet scan` via `cli.py:1809` → `:2147-2148`; `collisions` (kind `COORDINATE`) via `cli.py:2443` → `:2952-2957`; `ownership_rank` (`graph/collisions.py:165-181`); §3.1 step 3; §3.1 step 8; §12.27. **[Corrected 2026-08-28, round-L lane W8, base `a13a971`; `src/fleet/cli.py` line numbers re-derived against `a8f52c9` by round-L lane W9, `cli.py` having gained 5 lines at `:1759` since the base — each re-derived by matching the cited text, not by adding 5: this row's Detection cell read *“`coordinates.coord_key` PK conflict on insert”* and its Mitigation cell began *“`ON CONFLICT DO NOTHING` + `collisions` row”* and contained the clause *“loser's key suffixed”*; all three are quoted here so a sweep for them finds this correction and not a survival. Three measurements at `a13a971`. **(1)** The upsert is `ON CONFLICT (coord_key) DO UPDATE SET owner_repo_id = COALESCE(…)` (`src/fleet/cli.py:1602-1608`), so the row specified here is not the row the code writes. **(2)** A PK conflict is the wrong detection signal in any case: `coordinates` has no `run_id` (`state/schema.sql:113`, bare `coord_key` PK) while `collisions` is run-partitioned (`:343-355`), so a conflict means *seen before* and would fire on every rescan; the detector reads `manifests` instead, for the reason `cli.py::_coordinate_claims` (`:2443-2457`) states — by the time two publishers reach `coordinates` the contest is already unobservable there. **(3)** The row bundled two mechanisms the implementation has split across two commands; the split is what the cells above now state. The `edges.ambiguous` half was already live at scan time before round K; only the collision-row half was unwired, and `3da6e79` wired it into `fleet sequence`. **The struck `loser's key suffixed` clause is struck, not repointed: nothing implements it.** A whitespace-normalised sweep of `src/` for `suffix` finds no writer that suffixes `coordinates.coord_key` — the only suffixing in `graph/collisions.py` is `_file_collisions`' path suffix (`:402-410`), a different key space. §3.1 step 3 clause (iii) still states it as design; whether it should be built is an **adjudication** this marker does not make, and a reconciler must not add a suffixing writer merely to make the retired clause true.]** |
...
| 10 | **Crash mid-mutation** — `SIGKILL` between "patch applied" and "row written" | Two Git-local conditions, and there is no third: a `RUNNING` task **with** a commit on `migrate/<repo>` carrying its `Fleet-Task-Id`, or **without** one (plus, possibly, a dirty worktree) | **The commit is the record (ADR-0024).** `git commit` writes one tree covering every changed file and moves the branch ref by `flock`+`rename(2)`, so a *partially applied* multi-file patch is not a state the branch can reach — only the disposable worktree can be dirty. Resume asks Git which of the two cases holds and **corrects the SQLite row to match**: commit present → mark the task done with that SHA, re-run nothing; commit absent → `git reset --hard <tasks.pre_commit_sha>` + `git clean -fdx` (the TASK anchor — the phase anchor would delete earlier `DONE` tasks' commits, §3.2 step 6.5) and re-run the rung. No attempt is consumed either way (`TRANSIENT_INFRA`). Re-application is blocked by the `Fleet-Patch-Id` trailer, which lives in the same object as the change it names | `vcs/commits.py` (`find_task_commit`, `discard_task`); `Fleet-Task-Id` / `Fleet-Patch-Id` trailers; `tasks.pre_commit_sha`; `phases.base_ref`; `attempts.commit_sha`; §3.2 step 6; §11.5 step 4; §12.15, §12.45; row 41. **Marker 2026-09-01 (D89 Phase 2 Task B, fix wave):** "two Git-local conditions, and there is no third" is per-`task_id` and still exactly true read that way. As of this change it is no longer the whole picture for the REWRITE kind: a REWRITE coarse row is now asked about per-unit, and a row whose units land only partially gets a genuine third verdict, "partially landed" (ADR-0103). The original cell text is left as written per this project's "annotate, never rewrite" convention |
