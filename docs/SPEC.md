...
otherwise.** Where a backend exposes adaptive/extended thinking the harness uses it, and that
forbids temperature pinning; where a backend exposes `temperature`/`seed` the harness still does not
