...
tree-SHA comparison, because a commit is either on the branch or it is not (§3.2 step 6) — **a claim about Git's *verdict*, and not about a question Git could not be put.** A candidate whose worktree is gone, whose branch or anchor does not resolve, or whose probe did not settle is reported unreconciled with its reason and its row left byte-for-byte as it was; folding that into either branch would mark a task `DONE` carrying a SHA nobody found, or `reset --hard` a tree that may hold the only copy of the work (ADR-0087 §3);
