...
7. **Manifest adapter completeness.** All six shipped `ManifestAdapter`s parse their fixture manifests under `tests/fixtures/repos/` and produce `Coordinate`s whose `key` matches the fixture's expected value. **Disclosed 2026-08-28 (round-K repair of `12be741`, lane W4):** **the property holds; the named location does not exist as described, and this criterion is not a passing gate as written.** At `12be741` `tests/fixtures/repos/` contains exactly one file, `.gitkeep`, and a sweep of `src/` and `tests/` for that path returns **zero** references; the adapters' manifests are synthesized inline into `tmp_path` by `tests/test_manifests.py`'s `write()` helper (`:47-50`). The 2026-08-27 audit listed this criterion in its own unsatisfiable class and then corrected neither it nor §12.17 — this marker closes that half. Whether committed fixture repos are wanted at that path is an **adjudication**, not a correction; do not create them merely to make this sentence true.
