...
correct. §38 stays **partial, now blocked on D115 (branch-topology) and D105
(`--repoll-prs`-interaction reliability)** — D94 itself is no longer a listed blocker.
...
| OPEN — blocked on an existing D-number, don't duplicate | 3 | 31 (blocked on new `D111`, confirmed multi-leg by round VI research-22, 2026-09-03 — added to this row, see §31's own entry), 38 (partial — round VI task 45 closed D94 itself; now blocked on `D115` (a newly-found branch-topology gap: this monorepo's ingest never creates a per-repo `migrate/<repo>` branch, so D94's mechanism has no live path to run against in production) and `D105` (the `--repoll-prs`-interaction reliability gap), see that entry), 43 (partial) — `36` removed 2026-09-04 after round VI task 46 closed it, moved to the DONE row above; `22` removed 2026-09-03 after round VI tasks 26-29 closed it, moved to the DONE row above |
