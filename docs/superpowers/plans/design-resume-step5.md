...
| 9 | **Settle and record the step-5 flag semantics** — the un-refusal itself ships with row 10 | S | 7, 8 | `docs/DECISIONS.md` (ADR-0079), this file | ADR-0079 rules `--from-phase` as **option C(i)**, a repo filter: the floor `orchestrator.reentry.phase_floor` computed is never overridden, clamped or capped, and a repo whose floor sits below the flag is **skipped** rather than re-aimed. It rules `--repo` a **consistency ruling** against the six sibling verbs that already declare it (`plan`, `build`, `verify`, `migrate_repos`, `transform`, `pr` — measured by AST at `f4eade0`), scoping §11.5 steps 5, 6 and (later) 8 and **never** the run-wide steps 2, 3, 4 and 7; the same scope governs `--from-phase`. It records as a recital that `--revalidation` / `--raise-revalidation-rounds` **stay refused** (ST1's §13 row 34 and §3.5.1's stub lifecycle, not the re-entry floor). **`--reset-attempts`' semantics are NOT ruled and stay open** (ADR-0079 §8). **Nothing is removed from `_refuse_unbuilt_resume_flags`' refusal set by this row**: with step 8 the sole remaining absence **in `docs/SPEC.md` §11.5's numbered list**, no ruled flag has a continuation to scope, so the un-refusal ships with row 10 (ADR-0079 §6). *(Corrected 2026-08-22, W24 at ADR-0079's own commit. This row previously read "**Un-refuse the step-5 flags**", S, deps 7, 8, files "`src/fleet/cli.py` (`_refuse_unbuilt_resume_flags`), `tests/test_cli.py`", criterion "`--from-phase` / `--repo` / `--reset-attempts` removed from the refusal set with their semantics fixed by an ADR (ambiguity 4); `--revalidation` / `--raise-revalidation-rounds` stay refused (they are ST1's row 34, not step 5)". The second clause survives as a ruling; the **removal** is what ADR-0079 §6 falsifies, which is why the title and the file list move with it. The retired text is quoted here on purpose, so a count-based sweep for it finds this quotation — that hit is the retraction, not a survival. Row 10 still does not name the un-refusal in its own criterion; that is reported to the dispatcher rather than edited here.)* *(Scoped 2026-08-22, W35 at `9bf15bb`. This criterion said "with step 8 the sole remaining absence" with no scope; that holds of `docs/SPEC.md` §11.5's numbered list and is false of §10's `fleet resume` row, which orders **`stub_reconcile`** between the `blocked_by` recompute and the projection — a ninth absence, recorded as **D80** in `docs/INTEGRATION_HONESTY.md` and unowned. Measured at `9bf15bb` and again at `f36c9ad` by `ast` over all 115 `.py` blobs under `src/`: calls named `reconcile`/`stub_reconcile` = **0**, imports of any module named `stubs` = **0**. **ADR-0079 §6's ruling is unaffected and holds *a fortiori*** — two absences mean even less for a scoping flag to scope, so the un-refusal still ships with row 10 and this correction is not grounds to reopen it.)* |
| 10 | **Step 8 — "continue": delegate to the phase composition roots in phase order, and un-refuse the two flags ADR-0079 ruled** | L | 9 | `src/fleet/cli.py` (`_refuse_unbuilt_resume_flags`), `tests/test_cli.py`, `docs/DECISIONS.md` | `fleet resume` without `--dry-run` runs `_transform_impl` → `_build_impl` → `_verify_impl` for the phases the floors demand and stops at the first halt. **It exits 0 only when no delegate halted**: a halting delegate's exit code propagates through `cli._raise_for_phase`, which is `docs/SPEC.md` §10's exit-code mapping read off the payload and must not be copied a fourth time; `ResumeIncompleteError` is deleted from `resume()`; **no new `PhaseRunner` instantiation is added anywhere**. **The un-refusal ships here**: `cli._refuse_unbuilt_resume_flags`' `unbuilt` dict loses **`--from-phase`** and **`--repo`**, whose semantics are **ADR-0079 §2** (option C(i), a repo filter) and **ADR-0079 §4** (a consistency ruling scoping §11.5 steps 5, 6 and 8) — cited here, deliberately not restated, because a second copy is a second site to keep in step. **The other three entries do not move with them**: `--revalidation` and `--raise-revalidation-rounds` stay refused (ADR-0079 §5 — a §3.5.1 stub-lifecycle question, not a re-entry-floor one), and `--reset-attempts` stays refused because ADR-0079 §8 expressly does **not** rule its semantics, so no row owns removing it until an ADR does. *(Extended 2026-08-22, W27. Nothing above is retracted; this is an addition. Row 9's rewrite at `bd35a54` moved the un-refusal out of row 9 per ADR-0079 §6 without any row taking it on, so the plan had no row naming who edits the refusal set — reported by W24 rather than widened into its own path set, and this criterion now owns it. Measured at `bac5069` under `.venv/bin/python`, by parsing the `unbuilt` dict out of the `git show bac5069:src/fleet/cli.py` blob with `ast`: it has **five** keys and this row removes **two** of them. The dispatch routing this fix said the row must own removing all five; **ADR-0079 §5 and §8 falsify that**, and this criterion is written to the ADR rather than to the dispatch. `tests/test_cli.py` joins the file list because `test_resume_refuses_the_flags_whose_behaviour_does_not_exist` invokes `resume` with `--from-phase 2` and asserts exit 2 — measured at the same ref, not assumed.)* *(Corrected 2026-08-22, W35 at `9bf15bb` — one clause fixed, one left explicitly OPEN, one class homed here, one reported and deliberately not touched. **(i) FIXED — "exits 0" was unqualified and, read literally, mandated a defect.** `cli._raise_for_phase` raises `HumanInterventionError` on exit 7 and `FleetCliError(exit_code=code)` on **every** non-`SUCCESS` code — read from the `git show HEAD:src/fleet/cli.py` blob at `9bf15bb` — and its own docstring calls itself *"One exit-code mapping for both phase verbs — §10's table"*. A step 8 that exited 0 at a halt would ship a green exit for a fleet that stopped, and writing its own mapping would be a fourth copy of that one — "fourth" measured, not assumed: by `ast` over the `HEAD` `cli.py` blob at `9bf15bb`, the functions pairing a `raise HumanInterventionError` with a `raise FleetCliError(exit_code=…)` are exactly **3** (`scan`, `transform`, `_raise_for_phase`), reproducing R4's count by an independent walk. This was corrected rather than left open because a primary source settles it. **(ii) OPEN, NOT DECIDED — "the three composition roots" does not cover every floor this row consumes.** `orchestrator.reentry.phase_floor` can return `Phase.SCAN`: exercised at `9bf15bb` under `.venv/bin/python` with `fleet.__file__` asserted against this checkout's `src/`, `phase_floor({}, {})` returns `Phase.SCAN` and so does `phase_floor({p: None for p in Phase}, {})`; `docs/SPEC.md` §11.5 step 5 says the same in prose. `_transform_impl`, `_build_impl` and `_verify_impl` cannot serve a `SCAN` floor; `_scan_impl` exists and takes `only`. **No primary source settles what step 8 does with such a repo, so this lane names the options rather than inventing a semantics.** **D-1** — a fourth root, `_scan_impl` first; its flag surface (`skip_symbols` / `skip_classify` / `skip_contracts` / `concurrency`) has no resume equivalent, so step 8 must choose defaults an operator never wrote — the defect shape `models/tasks.py::BackendTarget.effort` records. **D-2** — three roots; a `SCAN` floor is reported in the payload and skipped, which makes §10's *"continue from each repo's re-entry floor"* false for that repo. **D-3** — three roots; a `SCAN` floor refuses the continuation with a named exit, halting the other repos too. The row title now reads "the phase composition roots" instead of "the three", so it no longer pre-decides the count by omission; whoever rules this must fix the title, this clause and `docs/SPEC.md` §10's row in **one** change. **(iii) HOMED HERE — ADR-0079 §7a's 2-site class**, *"a surface of `fleet resume` states a reason for refusing `--revalidation` / `--raise-revalidation-rounds` that is false for them"*. Both sites are `_refuse_unbuilt_resume_flags`' docstring and its `UsageError`, the exact function this row already edits; re-measured as still live at `9bf15bb` from the `HEAD` blob. §7a assigned it to "subtask 9d", which rewritten row 9 does not contain — token `9d` in this file: **0**. This row's un-refusal edit must also stop that message giving the revalidation pair a reason that is false for them. **(iv) REPORTED, NOT EDITED** — this criterion says "`CommandUnavailableError` is deleted from `resume()`". At `9bf15bb` `resume()` raises `ResumeIncompleteError` (measured by `ast` over all 115 `src/` blobs: 1 class def, 1 `ast.Raise`, 1 `ast.Name` load) and `CommandUnavailableError` is a different, still-raised class whose own docstring distinguishes the two. That clause is left byte-identical for its owner — ADR-0076's standing bullet — rather than rewritten by this lane.)* *(Corrected 2026-08-22, W38 at `f127680`, resolving W35's **(iv)**. Verified by symbol, not by W35's or the dispatch's reading, by `ast` over `resume()`'s own body in the `src/fleet/cli.py` blob at `f127680`: it raises exactly two exception classes directly — `PrEmissionError` (a forge failure, unrelated to this clause) and `ResumeIncompleteError` (naming steps 6 and 8 absent, per its message) — and `CommandUnavailableError` is not one of them. `CommandUnavailableError`'s only raise site in `src/` remains `_unavailable` (per that class's own docstring, unchanged at this ref). The live criterion above is corrected to name `ResumeIncompleteError`. **This does not change what row 10 owes.** ADR-0076's final bullet ("`ResumeIncompleteError` should be deleted, not repurposed") already defers that deletion to subtask 10, and ADR-0089 §6 states it explicitly: *"It does not delete `ResumeIncompleteError`. That is subtask 10's, and ADR-0076's bullet stands."* The correction states the obligation row 10 already carried; it is a wording fix, not a re-scope, so nothing is reported to the dispatcher as an obligation change. **Class swept, not just the site.** Whitespace-normalised over this file before this correction: `CommandUnavailableError` occurs 4 times, `ResumeIncompleteError` occurs 1 time. Of the 4 `CommandUnavailableError` sites: this clause (the false one, now fixed above); the `(iv)` quotation directly above, inside W35's own annotation, quoting the retired text on purpose so a count-based sweep finds the quotation rather than a survival — left byte-identical as history; the docstring-difference sentence immediately following it ("`CommandUnavailableError` is a different, still-raised class...") — true, untouched; and the "Order inside `_resume_impl`" step-7 line, anchored at commit `4a519b3` (ADR-0076's own "the landing this corrects"), which truthfully describes that historical commit's code and is correctly left alone. **Result: 1 genuine false claim in this file; 0 remain after this fix.** A tracked-tree sweep (`git grep -l CommandUnavailableError`, `git grep -l ResumeIncompleteError`, union, minus this file) finds **16** other tracked files: `docs/DECISIONS.md`, `docs/INTEGRATION_HONESTY.md`, `docs/PROGRESS.md`; **ten** `docs/superpowers/plans/*.md` files (`design-resume-step5-orchestrator-ledger.md`, `design-resume-step5-review-2-phase-floor.md`, `design-resume-step5-review-final.md`, `handoff-round-e.md`, `ledger-sdd-backlog-b.md`, `open-items-audit-round-b.md`, `open-items-audit-round-c.md`, `resume-step5-subtask-7-research.md`, `resume-step5-subtask-9-research.md`, `resume-step5-subtasks-4-6-research.md`); and `src/fleet/cli.py`, `tests/test_cli.py`, `tests/test_floor_rule_statements.py` — all outside this lane's edit scope. Each occurrence checked there is either historical (anchored at a stated commit predating ADR-0076, e.g. `ledger-sdd-backlog-b.md`, `resume-step5-subtasks-4-6-research.md`) or already states the correct name (`resume-step5-subtask-7-research.md` already documents this exact false claim, citing it rather than repeating it) — none reported as a further live defect.)* |
