...
| 11 | **VALID** | `_target_for` (`cache.py:621-628`) matches `(target.backend, target.model_id)` and **returns the first match**, so a route carrying a primary and a standby that differ *only* in `effort` resolves to the primary and `_store_response` (`:593`) stores the primary's effort for an answer the standby produced. `usage` carries no effort, so the ambiguity is not recoverable at that line. Not fixed; only documented (`INTEGRATION_HONESTY.md:3602-3607`). **Severity note not in §38:** the shipped `config/models.yaml` has six targets with six distinct `model_id`s, so no current route can trigger it — it is a latent code defect, not a live one. **Falsified 2026-08-21 by `9555346`; marker added by lane W19 (round E), with the row above left exactly as it was written.** The configuration this row describes — *"a route carrying a primary and a standby that differ only in `effort`"* — is now refused at router construction by `TierRoute._one_effort_per_backend_and_model_id` (`src/fleet/llm/client.py`), so *"Not fixed"* no longer holds; the decision, and the previously-legal configuration it removes, are **ADR-0091**. The row's severity note **still reproduces** at `704099c` — six shipped targets, six distinct `model_id`s, **one** target per tier, so no shipped route can fail over at all — which is why the remedy was to refuse the config rather than to change the key. Anchors: the range `cache.py:621-628` is kept as history and no longer bounds that function (the same commit grew its docstring by seven lines, so it now runs to `:635`) — cite **`cache._target_for`**; `cache.py:593` **still resolves** at `704099c` and is left alone; `INTEGRATION_HONESTY.md:3602-3607` has rotted — the leg it points at is D61's *"Second consequence"* paragraph. **Not falsified, and deliberately not marked as such:** that D61 leg itself — a *served* `model_id` losing the match, so `effort` falls back to the primary's — re-measures **live** at `704099c` and is a different door onto the same symptom. |
...
| 6 | §13 row 43 — rate limiting (LARGE) | **Still right, and its scoping premise re-measures correct:** `limits.for_tier(...)` is acquired at exactly **one** call site in all of `src/` — `workers/classify.py:162` — out of 12 workers. `asyncio.Semaphore` is still constructed fixed-size at `budgets.py:979`. *(Both halves re-measured 2026-08-20, §39 — **item 6 is still right and still LARGE**, but neither premise reads as written any more. The denominator is **5**, not 12: only five workers touch `ctx.llm`, and neither `llm/client.py` nor `llm/calls.py` acquires anything. And `Limits.create` no longer constructs an `asyncio.Semaphore` for the LLM tiers — it constructs ADR-0083's `ResizableLimiter`, which resizes safely while slots are held, so **do not scope building a resizable ceiling; it exists.** Per D55: premise corrected, defect fully open, **0% closed** — `resize()` has no production caller in `src/` and no AIMD controller reads 429s. The remaining scope is the controller, the 429→`resize()` hop, and widening the acquisition beyond `classify`.)* |
