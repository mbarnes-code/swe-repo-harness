...
| # | paraphrase | verdict | evidence |
|---|---|---|---|
| 13 | retry semantics: ladder, ceiling, atomicity, transient budget | PARTIAL | COVERED: ladder-length/max_attempts mismatch IS a ValidationError at construction (test_state_models.py:1120-1142, plus a [1,8] sanity rail); transient failure increments `transient_retries` leaving `attempts` unchanged (test_retry.py). Tier escalation to `["DETERMINISTIC","LLM_REPAIR","LLM_ESCALATION"]` + `("REQUIRES_HUMAN_INTERVENTION", 3)` proven at PhaseRunner+RetryPolicy level against a real SqliteStateRepository. NOT: the 5-rung variant — `max_attempts=5` occurs ZERO times in tests/, and D50 documents `transform.ladder.*` as KNOWN_INERT (`LadderState(...)` at runner.py:512 takes no `ladder=` kwarg), so a configured ladder never reaches a rung. The "same statement" atomicity claim is asserted by END STATE only (test_repository.py:259-301) — **nothing is ever interrupted between the increment and the terminal write**, so it cannot distinguish one atomic UPDATE from two that both happened to finish. |
...
> **[Marker 2026-08-28, round-K lane W5 — measured at `12f2cf3`. The paragraph above is left exactly
> as its author wrote it; this is the annotation, not a rewrite.]** **Two thirds of that sentence
> went stale at `827fc6e`** — the commit that appended this annex and did not annotate this line, so
> a reconciler reading it alone concludes the initializer sentence still stands in §12. It does not.
> **§12.47's sentence is no longer standing:** `docs/SPEC.md:7435` now states it "is RETIRED, not
> deferred". **§12.28 was never the same sentence:** `docs/SPEC.md:7416` now carries a marker saying
> it is not the `D86` clause and must not be retired with it — it requires only that pool children
> carry no database handle, which is true in the tree. **The last third is still accurate:** the
> three `src/fleet/ecosystems/base.py` docstrings (`:109`, `:118` inside a raised message, `:632`)
> do still assert the mechanism unqualified, and that is `D86`'s remaining open leg.
