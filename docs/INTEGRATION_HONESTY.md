...
2. `record_attempt` (`state/repository.py:2326-2394`) passes `row.stdout_tail`/`row.stderr_tail`
   ...
   Production caller `_AttemptWriter.record` (`cli.py:6945`) sets
...
(`cli.py:13202-13439`), not the pre-Task-B code the paragraph above was describing.** *(Repointed
...
`_persist_scan_edges` (`cli.py:2276`), itself reachable only from the scan path (`cli.py:1876`).
...
time because no contract has been hoisted yet. Separately, `_sequence_impl` (`cli.py:3198-3366`)
...
`_continue_impl` (`src/fleet/cli.py:9838-9951`) returns early at `if not servable: return result`
