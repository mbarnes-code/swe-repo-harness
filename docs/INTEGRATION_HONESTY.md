...
`_persist_scan_edges` (`cli.py:2203`), itself reachable only from the scan path (`cli.py:1876`).
...
time because no contract has been hoisted yet. Separately, `_sequence_impl` (`cli.py:3053-3221`)
...
`_continue_impl` (`src/fleet/cli.py:9591-9704`) returns early at `if not servable: return result`
