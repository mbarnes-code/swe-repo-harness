...
correction to the finding it sits beside. `_transform_payloads` (now `cli.py:5329`, round VI task
...
`_run_verify_wave` (`cli.py:9368`), and `_prepare_verify` is the heavier git mutator of the two: under the integration
...
2. `record_attempt` (`state/repository.py:2483-2551`) passes `row.stdout_tail`/`row.stderr_tail`
   ...
   Production caller `_AttemptWriter.record` (`cli.py:7346-7418`, moved by round VI task 58's
...
(`cli.py:14242-14479`), not the pre-Task-B code the paragraph above was describing.** *(Repointed
2026-09-06/07, round VI, SEVENTEEN separate times now as this round's own successive `cli.py`
...
onto `main`, and now task 67's own two branch commits (`d548b38`'s stub-creation-decision landing
and `6d8961e`'s fix round, neither yet merged to `main` — this repointing is against task 67's own
branch tip, per the controller's re-review). Correction to this
...
(moved by round VI task 58's `cli.py` insertions: `cli.py:4737-4945`, `_TransformSink`'s
...
`_persist_scan_edges` (`cli.py:2294-2358`), itself reachable only from the scan path (`cli.py:1876`).
...
(`cli.py:3228-3437`, moved by round VI task 58's `cli.py` insertions) does reach a real hoist via
...
`_continue_impl` (`src/fleet/cli.py:10585-10698`, moved by round VI task 58's `cli.py`
...
whole codebase; `_persist_contract_edges` (`cli.py:3574-3622`, moved by round VI task 58's
...
`_graph_edges` (`cli.py:3863-3909`) is widened with the identical `contracts.status IN
