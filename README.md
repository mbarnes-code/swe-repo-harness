...
`config/` and are present in a fresh checkout; keep `repos.yaml` and `fleet.yaml` as shipped unless
you need overrides. Values have typed defaults, but the files still need to exist at those paths.
