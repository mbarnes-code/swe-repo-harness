"""ADR-0145 — GPT-OSS's `apply_patch` patch-language parser, vendored from
`references/gpt-oss/gpt_oss/tools/apply_patch.py` (upstream source, per that file's own header
comment: https://cookbook.openai.com/examples/gpt4-1_prompting_guide). `references/` is read-only
(CLAUDE.md §5); this is a fresh copy under `src/`, not an import of the reference tree.

Adaptations from upstream, and nothing else (Rule 3 — surgical vendoring):
  * `typing.Dict`/`List`/`Optional`/`Tuple`/`Union` -> builtin generics / `X | None`, required by
    this repo's `ruff` `UP` (pyupgrade) rule family (`pyproject.toml:161`) under
    `requires-python = ">=3.12"` (`pyproject.toml:10`). Purely syntactic under
    `from __future__ import annotations`; no runtime behaviour changes.
  * The `main()` CLI entry point and its `if __name__ == "__main__":` guard are dropped — nothing
    in this harness invokes this module as a script.
  * Everything else — every dataclass, every function name and signature, the parsing algorithm,
    the fuzz-tolerance passes in `find_context_core` — is unchanged from upstream.

**Invariant this module must never violate:** `git apply` is the harness's ONLY writer into a
worktree (`src/fleet/vcs/git.py:435`, SPEC §3.2 step 6.6). `apply_patch()`/`apply_commit()` and
their default `write_file`/`remove_file`/`open_file` callables below DO write directly to the
filesystem — that is what upstream gpt-oss uses them for in a standalone CLI. Nothing in this
harness may call them with real filesystem callables: `src/fleet/llm/backends/harmony_gpt_oss.py`
calls only the pure, in-memory `text_to_patch()` + `patch_to_commit()` (never `apply_commit()`),
and this file's own tests are the ONLY callers of `apply_patch()`/`apply_commit()`, exercised
against a `tmp_path` to prove parity with upstream, never against the real worktree.
"""
...
class Parser:
    ...
    def is_done(self, prefixes: tuple[str, ...] | None = None) -> bool:
        return (
            ...
            or (
                bool(prefixes)
                and len(prefixes or ()) > 0
                and self._norm(self._cur_line()).startswith(prefixes or ())
            )
        )
