...
def _preexisting_overwrite_note(path: Path, text: str) -> str | None:
    ...
    Final-review Important 3: `None` also when the EXISTING file itself starts with
    `GENERATED_HEADER` — i.e. it was harness-generated by a previous attempt, not hand-written.
    `BuildgenWorker.run()` recomputes its unit list on every call, so `BUILD_BAZEL_UNIT`/
    `MODULE_BAZEL_UNIT` re-execute on re-entry, and a re-render can legitimately differ from a
    prior attempt's own output (a changed dep, a different MVS selection, a different
    `build_authoring` LLM answer). Without this check, that ordinary re-entry was misreported as
    "replaced a pre-existing hand-written Bazel file" — a false disclosure, since both the
    replaced and replacing bytes were harness output. A real hand-written file never starts with
    this header, so the discrimination is exact.
    """
    if not path.exists():
        return None
    try:
