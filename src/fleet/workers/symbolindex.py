...
Charge', ...)` in a Python `*_pb2_grpc.py` stub, and the same literal shape in Go/Java/TS/JS
stubs. The captured group is exactly `package.Service` — the same FQN `_proto_symbols` emits for
`GRPC_SERVICE`, so no reshaping is needed for `_api_contract_edges`'s fqn-equality join."""
