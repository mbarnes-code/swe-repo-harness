...
# =======================================================================================
# security finding #7 (CRITICAL) — write side: a tracked symlink is refused, never written through
# =======================================================================================
def test_materialize_refuses_to_write_a_support_file_through_a_tracked_symlink(tmp_path) -> None:
    """A source repo can commit a tracked symlink at exactly the path a generated `SupportFile`
    (or `BUILD.bazel`/`MODULE.bazel`) would land at. `Path.write_text()` follows an existing
    symlink and overwrites whatever it points at, so without a guard `materialize()` would clobber
    any absolute host path the harness process can write, with no diff and no finding — the write
    side of the same primitive `rewrite.py`'s `is_symlink()` guard closes on the read side.

    `buildgen._write()` must refuse loud, before the write, with `UnsafeSourcePathError` — and the
    symlink target's content must be provably unchanged afterwards, not merely "an exception was
    raised somewhere".

    DISCRIMINATES: reverting the `is_symlink()` guard in `buildgen._write()` makes this test fail
    — `pytest.raises` sees no exception, and the sentinel assertion below fails too, because the
    old code silently overwrites the sentinel with the generated text.
    """
    root = tmp_path / "root"
    root.mkdir()
    outside = tmp_path / "outside-secret.txt"
    outside.write_text("SENTINEL_SECRET_CONTENT\n", encoding="utf-8")
    link_path = root / "pkg" / "BUILD.bazel"
    link_path.parent.mkdir(parents=True)
    link_path.symlink_to(outside)

    with pytest.raises(UnsafeSourcePathError, match=r"BUILD\.bazel"):
        materialize(root, [SupportFile(path="pkg/BUILD.bazel", content="GENERATED BY fleet\n")])

    assert outside.read_text(encoding="utf-8") == "SENTINEL_SECRET_CONTENT\n", (
        "the symlink target must be untouched — the old code overwrites it with the generated text"
    )


async def test_buildgen_worker_refuses_a_tracked_symlink_at_the_build_bazel_path(tmp_path) -> None:
    """The worker-level path (not just the bare `_write()`/`materialize()` functions): a symlink
    tracked at `<dest>/BUILD.bazel` must make `BuildgenWorker.run()` itself raise
    `UnsafeSourcePathError` rather than return a result that silently clobbered the symlink target
    — `classify_exception` (asserted generically in `tests/test_workers_base.py`) then maps this to
    `FailureClass.UNSAFE_SOURCE_PATH` via `BaseWorker.execute()`'s generic exception handler, with
    zero new plumbing (security finding #7, write side).
    """
    ecosystems.discover()
    ...
    outside = tmp_path / "outside-secret.txt"
    outside.write_text("SENTINEL_SECRET_CONTENT\n", encoding="utf-8")
    build_path = tmp_path / dest / "BUILD.bazel"
    build_path.symlink_to(outside)

    with pytest.raises(UnsafeSourcePathError, match=r"BUILD\.bazel"):
        await BuildgenWorker().run(make_ctx(tmp_path), payload)
