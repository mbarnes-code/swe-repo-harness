...
from fleet.ecosystems.contracts.thrift import ThriftContractAdapter
from fleet.models.enums import ContractKind, Ecosystem
from fleet.models.graph import ContractNode


def _thrift_node(identifier: str = "acme.rpc.v1", contract_id: str | None = None) -> ContractNode:
    return ContractNode(
        contract_id=contract_id or f"thrift:{identifier}",
        kind=ContractKind.THRIFT,
        identifier=identifier,
        source_paths=[{"repo_id": "acme-lib", "path": "rpc.thrift", "blob_sha": "deadbeef"}],
    )


def test_layout_ports_the_target_path_dotted_template() -> None:
    """Ports `workers/contracts.py`'s
    `TARGET_PATH[ContractKind.THRIFT] == "contracts/thrift/{dotted}"`."""
    adapter = ThriftContractAdapter()
    node = _thrift_node()

    assert adapter.layout(node) == Path("contracts/thrift/acme/rpc/v1")
...
def test_neutral_targets_emits_one_avro_library_with_relocated_basenames() -> None:
    ...
    target = targets[0]
    assert target.rule == "thrift_library"
    assert target.package == "contracts/thrift/acme/rpc/v1"
    assert target.name == "v1"
    assert target.srcs == ["rpc.thrift"]  # relocated basename, not the source repo's own path


def test_binding_target_depends_on_the_neutral_targets_label_and_uses_the_supplied_rule() -> None:
    """`rule` is supplied by the driver from `contract_bindings[THRIFT]` -- this adapter never
    looks up a language itself (SPEC §7.6's ABC docstring). `jvm.py:59` maps
    `ContractKind.THRIFT -> "java_thrift_library"` -- a REAL contract_bindings entry, unlike
    PROTO's test, which uses `Ecosystem.PYPI`."""
    adapter = ThriftContractAdapter()
    node = _thrift_node()
    neutral = adapter.neutral_targets(node)[0]

    binding = adapter.binding_target(node, Ecosystem.MAVEN, "java_thrift_library")

    assert binding.rule == "java_thrift_library"
...
def test_layout_raises_rather_than_silently_emitting_a_reserved_dest() -> None:
    ...
    adapter = ThriftContractAdapter()
    node = _thrift_node(identifier="_scc.demo", contract_id="thrift:reserved-collision-test")
