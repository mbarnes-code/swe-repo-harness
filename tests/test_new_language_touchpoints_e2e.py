...
from fleet.models.build import BuildPlan, BuildTarget
from fleet.models.enums import BreakStrategy, ContractKind, ContractStatus
...
from tests.fixtures.adapters.ruby_ecosystem import (
    _RubyEcosystemAdapterBase,
    make_ruby_ecosystem_adapter,
)
...
def _write_config(root: Path, sources: dict[str, Path], *, fleet_yaml: str = _FLEET_YAML) -> None:
    config = root / "config"
...
# -------------------------------------------------------------------------------------------
# Clause B fixture: acme-identity + acme-billing (verbatim from CYCLE_FLEET, a real npm cycle
# through `proto:acme.identity.v1`) plus `acme-gem`, a second consumer of the SAME contract whose
# `EcosystemAdapter.contract_bindings` omits PROTO (§12.34 Clause B / D113, ADR-0119)
# -------------------------------------------------------------------------------------------

#: `acme-gem`'s own fixture sources plus one checked-in GENERATED binding under `.../generated/**`
#: whose path contains `/acme/identity/v1/` — `workers/contracts.py::_generated_paths` (`:733-756`)
#: matches it against the contract's identifier and `_is_generated` (`:591-593`) accepts it via the
#: GLOB half of `scan.generated_globs` alone (`**/generated/**`), deliberately never depending on
#: whether the symbol indexer parses Ruby at all. That is what makes `acme-gem` a real *consumer*
#: of `proto:acme.identity.v1` (`workers/contracts.py::_consumers` adds any repo holding a
#: generated-path hit) with zero `src/fleet/` changes.
_RUBY_GEM_GENERATED_BINDING = (
    "# Generated by grpc-tools/protoc-gen-ruby. DO NOT EDIT.\n"
    "# @generated\n"
    "module Acme\n  module Identity\n    module V1\n    end\n  end\nend\n"
)

_CLAUSE_B_FLEET: dict[str, dict[str, str]] = {
    "acme-identity": dict(CYCLE_FLEET["acme-identity"]),
    "acme-billing": dict(CYCLE_FLEET["acme-billing"]),
    _REPO_NAME: {
        **_RUBY_FIXTURE_FILES,
        "lib/generated/acme/identity/v1/identity_pb.rb": _RUBY_GEM_GENERATED_BINDING,
    },
}

#: Same shape as `_FLEET_YAML` above, minus `graph: hoist_contracts: false` (hoisting is ON by
#: default — the configuration under test for Clause B). `build.ruleset_versions` REPLACES
#: `BuildSection`'s whole default dict rather than merging with it (ordinary pydantic-settings
#: behaviour for an explicitly declared field), so — unlike Clause A's Ruby-only fixture, which
#: never dispatches a real ruleset — this fleet's real npm/TS repos (`acme-identity`,
#: `acme-billing`) still need `aspect_rules_js`/`aspect_rules_ts` pinned or their own Phase 3
#: fails with "an unpinned bazel_dep is a build that resolves differently" (§9), unrelated to
#: this fixture's own `rules_ruby` pin for `acme-gem`. Carried over from `BuildSection`'s shipped
#: defaults verbatim, plus `rules_ruby`.
_FLEET_YAML_CLAUSE_B = """\
run:
  monorepo_path: ../acme-monorepo
  cache_dir: cache/
  work_dir: work/
...
def _make_clause_b_workspace(tmp_path: Path) -> Path:
    sources = {
        ...
        for name, files in _CLAUSE_B_FLEET.items()
    }
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    _write_config(workspace, sources, fleet_yaml=_FLEET_YAML_CLAUSE_B)
...
# -------------------------------------------------------------------------------------------
# 3. Clause B: ContractBindingUnavailable + BuildPlan.unbound_contract_kinds, driven end to end
#    (§12.34 second sentence / D113, ADR-0119) — a SECOND, later task's fleet, composed from
#    landed fixtures rather than an edit to `_make_ruby_workspace`'s own
# -------------------------------------------------------------------------------------------


def test_the_decoy_ecosystem_substitutes_through_the_nested_unbound_contract_kinds_annotation() -> (
    None
):
    """The decoy-enum hazard ADR-0119's task brief names but does not itself prove:
    `BuildPlan.unbound_contract_kinds: list[tuple[ContractId, Ecosystem]]` is a NESTED annotation,
    and `_decoy_ruby_ecosystem`'s `_substitute_ecosystem` must recurse through `typing.get_args` to
    rewrite the `Ecosystem` buried inside `list[tuple[...]]`, not just a bare `Ecosystem` field.

    If the sweep missed this shape, constructing a `BuildPlan` carrying the decoy member here
    would raise `ValidationError`: the decoy is a distinct `StrEnum` class, not `Ecosystem` itself,
    so pydantic's compiled core schema for an unswept field would reject it outright.
    """
    with _decoy_ruby_ecosystem() as ruby_member:
        plan = BuildPlan(
            unit_id=PROTO_ID,
            dest="proto/acme/identity/v1",
            generated_by="adapter",
            targets=[
                BuildTarget(package="proto/acme/identity/v1", name="identity", rule="proto_library")
            ],
            unbound_contract_kinds=[(PROTO_ID, ruby_member)],
        )
        assert plan.unbound_contract_kinds == [(PROTO_ID, ruby_member)], plan.unbound_contract_kinds


def test_contract_binding_unavailable_finding_and_unbound_contract_kinds_end_to_end(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """SPEC §12.34's second sentence, driven for real (D113, ADR-0119): `proto:acme.identity.v1`
    is hoisted out of a real npm cycle (`acme-identity`/`acme-billing`, verbatim from
    `tests.test_workers_contracts.CYCLE_FLEET`) and consumed by a SECOND ecosystem — the fixture
    Ruby adapter, whose `contract_bindings` has no `PROTO` entry. `fleet build` must still bind
    the npm consumer, emit exactly one `ContractBindingUnavailable` finding for the Ruby one,
    persist both outcomes in the contract's own `BuildPlan`, and exit `SUCCESS`.
    """
    with _decoy_ruby_ecosystem() as ruby_member:
        # -- anti-vacuity on the fixture adapter itself, before any drive ------------------
        assert ContractKind.PROTO not in _RubyEcosystemAdapterBase.contract_bindings, (
            "the fixture must really omit PROTO or this test proves nothing"
        )
        assert ContractKind.OPENAPI in _RubyEcosystemAdapterBase.contract_bindings, (
            "the fixture must really bind OPENAPI, per its own docstring"
        )

        workspace = _make_clause_b_workspace(tmp_path)
        monkeypatch.chdir(workspace)
        monkeypatch.setattr(budgets_module, "new_cpu_pool", _fake_cpu_pool)
        monkeypatch.setattr(cli, "new_cpu_pool", _fake_cpu_pool)
        ...
        run_id = str(query(workspace, "SELECT run_id FROM runs")[0][0])

        # -- anti-vacuity on the hoist itself, fail loudly on each -------------------------
        cycle_rows = query(
            workspace,
            "SELECT payload FROM findings WHERE run_id = ? AND kind = 'CycleDetected'",
            (run_id,),
        )
        assert len(cycle_rows) == 1, cycle_rows
        cycle_payload = json.loads(str(cycle_rows[0][0]))
        assert cycle_payload["break_strategy"] == BreakStrategy.CONTRACT_HOIST.value, cycle_payload
        assert cycle_payload["hoisted_contract_ids"] == [PROTO_ID], cycle_payload

        contract_rows = query(
            workspace,
            "SELECT status, hoist_target_path, consumer_repo_ids FROM contracts "
            " WHERE run_id = ? AND contract_id = ?",
            (run_id, PROTO_ID),
        )
        assert len(contract_rows) == 1, contract_rows
        status, hoist_target_path, consumer_json = contract_rows[0]
        assert status == ContractStatus.HOISTED.value, status
        assert hoist_target_path, "a HOISTED contract must carry a non-null hoist_target_path"
        consumer_repo_ids = json.loads(str(consumer_json))
        assert _REPO_NAME in consumer_repo_ids, (
            f"acme-gem must really land in contracts.consumer_repo_ids: {consumer_repo_ids}"
        )
        assert "acme-billing" in consumer_repo_ids, consumer_repo_ids

        # -- fleet build: the criterion's own "and the run still completes" clause --------
        make_monorepo(workspace)
        _install_fake_bazel_stack(monkeypatch, workspace)
        result = build(workspace, "--no-sandbox")
        assert result.exit_code == ExitCode.SUCCESS, result.output

        # -- exactly one ContractBindingUnavailable row, naming the contract and Ruby -----
        finding_rows = query(
            workspace,
            "SELECT repo_id, payload FROM findings "
            " WHERE run_id = ? AND kind = 'ContractBindingUnavailable'",
            (run_id,),
        )
        assert len(finding_rows) == 1, finding_rows
        finding_repo_id, finding_payload_json = finding_rows[0]
        assert finding_repo_id is None, finding_repo_id
        finding_payload = json.loads(str(finding_payload_json))
        assert finding_payload["contract_id"] == PROTO_ID, finding_payload
        assert finding_payload["ecosystem"] == ruby_member.value, finding_payload

        # -- the control that makes the above mean something: no finding names npm, and its
        # -- real binding target is present in the persisted plan (a pass that blanket-emitted
        # -- findings for every consumer would satisfy the assertion above and fail this one) --
        assert finding_payload["ecosystem"] != "npm", finding_payload

        plan_dir = workspace / "artifacts" / "build" / run_id
        plan_path = plan_dir / f"{slug(PROTO_ID)}.plan.json"
        assert plan_path.is_file(), sorted(p.name for p in plan_dir.iterdir())
        plan = BuildPlan.model_validate_json(plan_path.read_text(encoding="utf-8"))
        assert plan.unbound_contract_kinds == [(PROTO_ID, ruby_member)], plan.unbound_contract_kinds
        assert any(t.rule == "proto_library" for t in plan.targets), plan.targets
        assert any(t.rule == "ts_proto_library" for t in plan.targets), plan.targets
