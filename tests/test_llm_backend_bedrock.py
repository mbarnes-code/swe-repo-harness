...
def test_a_conversation_whose_first_non_system_turn_is_not_user_is_refused() -> None:
    ...
    "no turns at all" check) but opens on 'assistant', which `Converse` requires to be 'user'.
    Sending it anyway would have the service reject a well-formed-looking request outright, instead
    of failing loudly here where the field can be named (Rule 11)."""
    with pytest.raises(BedrockTargetMisconfigured, match="assistant"):
        build_request(
            target(),
            (
...
def test_usage_model_id_echoes_the_config_string_even_when_bedrock_reports_another() -> None:
    ...
    inference-profile ARN or a `us.`-prefixed cross-region id, and the "resolution" language around
    this field invited passing that through in three independent lanes. Doing so makes read key !=
    write key on EVERY call: a permanent, total cache miss across the whole fleet, silent and
    indistinguishable from a cold cache because `attempts.llm_cache_hit` simply stays 0. The
    invariant is pinned by this test rather than left to whatever the neighbouring prose says.
